...
flags.DEFINE_bool("do_predict_all", True, "Whether to run predict all.")
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids, input_span_mask,
  ...
  with tf.variable_scope("squad_classifier"):
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      def compute_loss(logits, positions):
        with tf.variable_scope("squad_compute_loss"):
...
def write_predictions(all_examples, all_features, all_results, n_best_size,
  ...
  for (example_index, example) in enumerate(all_examples):
    ...
    for (feature_index, feature) in enumerate(features):  # multi-trunk
      #print("featuresi.unique_id===",feature.unique_id)
...
def main(_):
  ...
  if FLAGS.do_predict:
    ...
    if FLAGS.do_predict_all:
      ...
      predict_path=os.path.join(FLAGS.output_dir, "predict")
      if not os.path.exists(predict_path):
        os.mkdir(predict_path)
      for global_step, filename in sorted(steps_and_files, key = lambda x:x[0]):
        all_results = []
        time.sleep(5)
        tf.logging.info("***** do predictions *****")
        prediction_result = estimator.predict(predict_input_fn, checkpoint_path=filename, yield_single_examples=True)
        time.sleep(5)
        tf.logging.info("***** processing predictions result*****")
        for result in prediction_result:
          if len(all_results) % 1000 == 0:
            tf.logging.info("Begin predictions  Processing example: %d" % (len(all_results)))
            time.sleep(5)
          unique_id = int(result["unique_ids"])
          tf.logging.info("Finish predictions unique_id: %d" % (len(all_results)))
          start_logits = [float(x) for x in result["start_logits"].flat]
          tf.logging.info("Finish predictions start_logits: %d" % (len(all_results)))
          end_logits = [float(x) for x in result["end_logits"].flat]
          tf.logging.info("Finish predictions end_logits: %d" % (len(all_results)))
          all_results.append(
              RawResult(
                  unique_id=unique_id,
                  start_logits=start_logits,
                  end_logits=end_logits))
          if len(all_results) % 1000 == 0:
            tf.logging.info("End predictions Processing example: %d" % (len(all_results)))
        time.sleep(15)
        tf.logging.info("***** Writing predictions *****")
        output_json_name = "dev_predictions_"+str(global_step)+".json"
        output_nbest_name = "dev_nbest_predictions_"+str(global_step)+".json"

        output_prediction_file = os.path.join(predict_path, output_json_name)
        output_nbest_file = os.path.join(predict_path, output_nbest_name)
      ...
      for result in estimator.predict(
        ...
        unique_id = int(result["unique_ids"])
        tf.logging.info("Processing unique_id: %d" % (len(all_results)))
        start_logits = [float(x) for x in result["start_logits"].flat]
        tf.logging.info("Processing start_logits: %d" % (len(all_results)))
        end_logits = [float(x) for x in result["end_logits"].flat]
        tf.logging.info("Processing end_logits: %d" % (len(all_results)))
        all_results.append(
            RawResult(
                unique_id=unique_id,
                start_logits=start_logits,
                end_logits=end_logits))
        tf.logging.info("Processing all_results.append: %d" % (len(all_results)))
