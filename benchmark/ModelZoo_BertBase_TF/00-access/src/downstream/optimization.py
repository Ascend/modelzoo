...
def create_optimizer(loss, init_lr, num_train_steps, num_warmup_steps, use_tpu):
  ...
  if tf.flags.FLAGS.npu_bert_loss_scale not in [None, -1]:
    ...
    optimizer = lso.NPULossScaleOptimizer(opt_tmp, loss_scale_manager, is_distributed=True if int(os.getenv('RANK_SIZE')) > 1 else False)
...
class AdamWeightDecayOptimizer(tf.train.Optimizer):
  ...
  def apply_gradients(self, grads_and_vars, global_step=None, name=None):
    """See base class."""
    with tf.variable_scope("optimizer/lamb/apply_gradients"):
      assignments = []
      for (grad, param) in grads_and_vars:
        with tf.variable_scope("apply_one"):
