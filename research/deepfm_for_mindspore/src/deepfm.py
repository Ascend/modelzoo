...
from mindspore import nn, Tensor, ParameterTuple, context
...
from src.utils import init_method, init_var_dict
...
class DeepFMModel(nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.fm_w = var_map["W_l2"]  # [self.vocab_size, 1]  random
        self.fm_b = var_map["b"]
        self.embedding_table = var_map["V_l2"]  # [self.vocab_size, self.emb_dim] random
        " Deep Layers "
        self.deep_input_dims = self.field_size * self.emb_dim + 1
        self.all_dim_list = [self.deep_input_dims] + self.deep_layer_dims_list + [1]
        self.dense_layer_1 = DenseLayer(self.all_dim_list[0], self.all_dim_list[1], self.weight_bias_init, self.deep_layer_act, self.keep_prob)
        self.dense_layer_2 = DenseLayer(self.all_dim_list[1], self.all_dim_list[2], self.weight_bias_init, self.deep_layer_act, self.keep_prob)
        self.dense_layer_3 = DenseLayer(self.all_dim_list[2], self.all_dim_list[3], self.weight_bias_init, self.deep_layer_act, self.keep_prob)
        self.dense_layer_4 = DenseLayer(self.all_dim_list[3], self.all_dim_list[4], self.weight_bias_init, self.deep_layer_act, self.keep_prob)
        " FM, linear Layers "
    ...
    def get_callback_list(self, model=None, eval_dataset=None):
        ...
        if self.train_config.eval_callback:
            ...
            if eval_dataset is None:
                raise RuntimeError("train_config.eval_callback is {}; get_callback_list() args eval_dataset is {}".format(
                                        self.train_config.eval_callback, eval_dataset))
