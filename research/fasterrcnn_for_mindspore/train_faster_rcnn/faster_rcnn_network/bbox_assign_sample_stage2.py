...
class BboxAssignSampleForRcnn(nn.Cell):
    ...
    def __init__(self, batch_size, num_bboxes, add_gt_as_proposals):
        ...
        self.add_gt_as_proposals_valid = Tensor(np.array(self.add_gt_as_proposals * np.ones(self.num_gts), dtype = np.int32))
        ...
        self.scalar_zero = Tensor(0.0, dtype=float16)
        self.scalar_neg_iou_thr = Tensor(self.neg_iou_thr, dtype=float16)
        self.scalar_pos_iou_thr = Tensor(self.pos_iou_thr, dtype=float16)
        self.scalar_min_pos_iou = Tensor(self.min_pos_iou, dtype=float16)
    ...
    def construct(self, gt_bboxes_i, gt_labels_i, valid_mask, bboxes, gt_valids):
        ...
        # If self.add_gt_as_proposals is true, add gt
