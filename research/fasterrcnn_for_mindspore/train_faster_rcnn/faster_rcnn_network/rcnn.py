...
class DenseNoTranpose(nn.Cell):
    def __init__(self, input_channels, output_channels, weight_init):
        ...
        self.weight = Parameter(initializer(weight_init, [input_channels, output_channels], mstype.float16), name="weight")
...
class Rcnn(nn.Cell):
    ...
    Examples:
        Rcnn(representation_size = 1024, num_classes = 81, \
    ...
    def __init__(self,
        ...
        cls_weight = initializer('Normal', shape = [num_classes, self.rcnn_fc_out_channels][::-1], dtype = mstype.float16)
        reg_weight = initializer('Normal', shape = [num_classes * 4, self.rcnn_fc_out_channels][::-1], dtype = mstype.float16)
        ...
        self.num_bboxes = cfg.rcnn_num_bboxes_one_batch * batch_size
        ...
        # teeeeeeeeeeest
        rmv_first1 = np.zeros((self.num_bboxes, ))
        rmv_first1[-1] = 1
        self.rmv_first_tensor1 = Tensor(rmv_first1.astype(np.int32))
		
    def construct(self, input, bbox_targets, labels, mask):
        ...
        #mask = self.cast(self.cast(mask, mstype.int32) * self.rmv_first_tensor1, mstype.bool_)
    ...
    def loss(self, cls_score, bbox_pred, bbox_targets, bbox_weights, labels, weights):
        ...
        bbox_weights = self.cast(self.onehot(bbox_weights, self.num_classes, self.on_value, self.off_value), mstype.float16)        
