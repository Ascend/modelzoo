...
class Faster_Rcnn_Resnet50(nn.Cell):
    ...
    def __init__(self, batch_size=2, rcnn_num_classes=81):
        ...
        self.num_classes = rcnn_num_classes
        ...
        #Backbone resnet50
        self.backbone = ResNetFea(ResidualBlockTorch,
                                  config.resnet_block,
        ...
        self.rpn_with_loss = RPN(self.train_batch_size,
        ...
        #Proposal
        self.proposal_generator = Proposal(self.train_batch_size,
                                           config.num_classes,
                                           config.use_sigmoid_cls)
        self.proposal_generator.set_train_local(True)
        self.proposal_generator_test = Proposal(config.test_batch_size,
                                           config.num_classes,
                                           config.use_sigmoid_cls)
        self.proposal_generator_test.set_train_local(False)

        #Assign and sampler stage two
        self.bbox_assigner_sampler_for_rcnn = BboxAssignSampleForRcnn(self.train_batch_size, config.num_bboxes_stage2, True)
        self.decode = P.BoundingBoxDecode(max_shape=(768, 1280), means=self.target_means, \
                                          stds=self.target_stds)

        #Roi
        self.roi_align = SingleRoIExtractor(config.roi_layer,
                                            config.roi_align_out_channels,
                                            config.roi_align_featmap_strides,
                                            self.train_batch_size,
                                            config.roi_align_finest_scale)
        self.roi_align.set_train_local(True)
        self.roi_align_test = SingleRoIExtractor(config.roi_layer,
                                            config.roi_align_out_channels,
                                            config.roi_align_featmap_strides,
                                            1,
                                            config.roi_align_finest_scale)
        self.roi_align_test.set_train_local(False)

        #Rcnn
        self.rcnn = Rcnn(config.rcnn_in_channels * config.roi_layer['out_size'] * config.roi_layer['out_size'],
        ...
        self.bbox_mask = Tensor(np.concatenate((self.ones_mask, self.zeros_mask, self.ones_mask, self.zeros_mask), axis = 1))
        self.nms_pad_mask = Tensor(np.concatenate((self.ones_mask, self.ones_mask, self.ones_mask, self.ones_mask, self.zeros_mask), axis = 1))
    ...
    def construct(self, img_data, img_metas, gt_bboxes, gt_labels, gt_valids):
        ...
        rpn_loss, cls_score, bbox_pred, rpn_cls_loss, rpn_reg_loss, rpn_loss_print = self.rpn_with_loss(x,
                                                                            img_metas,
                                                                            self.anchor_list,
                                                                            gt_bboxes,
        ...
        rcnn_loss, rcnn_cls_loss, rcnn_reg_loss, rcnn_loss_print = self.rcnn(roi_feats,
                                               bbox_targets,
                                               rcnn_labels,
                                               self.squeeze(self.cast(rcnn_masks, mstype.bool_)))
