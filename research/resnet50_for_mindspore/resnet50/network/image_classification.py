    ...
    def __init__(self, backbone_name, num_classes):
        ...
        for name, cell in self.cells_and_names():
            if isinstance(cell, nn.Conv2d):
                cell.weight.default_input = init.initializer(
                    ...
                    cell.weight.default_input.shape(), cell.weight.default_input.dtype())
            elif isinstance(cell, nn.BatchNorm2d):
                cell.gamma.default_input = init.initializer('ones', cell.gamma.default_input.shape())
                cell.beta.default_input = init.initializer('zeros', cell.beta.default_input.shape())
        ...
        for name, cell in self.cells_and_names():
            if isinstance(cell, backbones.resnet.Bottleneck):
                cell.bn3.gamma.default_input = init.initializer('zeros', cell.bn3.gamma.default_input.shape())
            elif isinstance(cell, backbones.resnet.BasicBlock):
                cell.bn2.gamma.default_input = init.initializer('zeros', cell.bn2.gamma.default_input.shape())



def get_network(backbone_name, num_classes):
    try:
        if backbone_name in ['resnet50']:
