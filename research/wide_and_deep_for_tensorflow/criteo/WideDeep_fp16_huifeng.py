...
from models.tf_util import build_optimizer, init_var_map, \
...
class WideDeep:
    ...
    def __init__(self, graph, dataset_argv, architect_argv, init_argv, ptmzr_argv,
        ...
        #with tf.variable_scope('FMNNMG') as scope:
            ...
            #self.eval_id_hldr = tf.cast(self.eval_id_hldr, dtype=tf.int32)
            ...
            with tf.control_dependencies(update_ops):
                ...
                #loss_scale_manager = FixedLossScaleManager(loss_scale=1000)
                #loss_scale_manager2 = tf.contrib.mixed_precision.ExponentialUpdateLossScaleManager(
                #        init_loss_scale=2 ** 32, incr_every_n_steps=1000, decr_every_n_nan_or_inf=2, decr_ratio=0.5)
                ...
                #loss_scale_manager2 = FixedLossScaleManager(loss_scale=1000)
                ...
                #self.wide_ptmzr = tf.contrib.mixed_precision.LossScaleOptimizer(self.wide_ptmzr, loss_scale_manager)
                if rank_size > 1:
                    self.wide_ptmzr = NPUDistributedOptimizer(self.wide_ptmzr)
                ...
                #self.deep_optimzer = tf.contrib.mixed_precision.LossScaleOptimizer(self.deep_optimzer, loss_scale_manager2)
                if rank_size > 1:
                   self.deep_optimzer = NPUDistributedOptimizer(self.deep_optimzer)
                ...
                #self.train_op = tf.group(self.wide_ptmzr)
    ...
    def forward(self, wt_hldr, id_hldr, act_func, keep_prob,
        ...
        for i in range(len(self.h_w)):
            if training:
                ...
                #hidden_output = tf.matmul(
                #    tf.cast(npu_ops.dropout(
                #        activate(act_func, hidden_output), keep_prob=keep_prob)*scale,dtype=tf.float16), 
                #    tf.cast(self.h_w[i], dtype=tf.float16)) + tf.cast(self.h_b[i]*scale,dtype=tf.float16)
                #hidden_output = tf.cast(hidden_output, dtype=tf.float32)
                #hidden_output = hidden_output/scale
            else:
                hidden_output = tf.matmul(
                    activate(act_func, hidden_output),
                    self.h_w[i]) + self.h_b[i]
                #hidden_output = tf.matmul(
                #   tf.cast(activate(act_func, hidden_output)*scale,dtype=tf.float16),
                #       tf.cast(self.h_w[i],dtype=tf.float16)) + tf.cast(self.h_b[i]*scale,dtype=tf.float16)
                #hidden_output = tf.cast(hidden_output, dtype=tf.float32)
                #hidden_output = hidden_output/scale
            # if batch_norm:
        ...
        #return tf.reshape(hidden_output, [-1, ]) + tf.reshape(wideout,[-1, ]) , vx_embed
...
class FMNN:
    def __init__(self,  _rch_argv, _init_argv, _ptmzr_argv, _reg_argv, mode, src, loss_mode='full'):
	    ...
	    #self.id_hldr = tf.cast(self.id_hldr, dtype=tf.int32)
