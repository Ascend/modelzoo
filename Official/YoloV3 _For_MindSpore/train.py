...
from src.object_detection.yolo_v3.yolo import yolov3_darknet53, YoloWithLossCell
from src.object_detection.yolo_v3.yolo import TrainingWrapper
from src.common.utils.logging import get_logger
from src.common.utils.average_meter import AverageMeter
from src.common.utils.yolov3_utils import load_backbone
from src.common.optimizers import get_param_groups
from src.common.lr_scheduler.warmup_step_lr import warmup_step_lr
from src.common.lr_scheduler.warmup_cosine_annealing_lr import warmup_cosine_annealing_lr, \
...
from src.common.utils.cloud_copy_cache import copy_mox_ckpt_log_to_s3
from src.common.utils.var_init import default_recurisive_init
from src.object_detection.yolo_v3.config import Config_yolov3
from src.object_detection.utils.batch_preprocess_true_box import batch_preprocess_true_box, batch_preprocess_true_box_single
from src.object_detection.utils.shape_record import Shape_record
...
def train(cloud_args={}):
    ...
    network = yolov3_darknet53(is_training=True)
    ...
    '''
    decay_params = list(filter(default_wd_filter, network.trainable_params()))
    non_decay_params = [i for i in network.trainable_params() if i not in decay_params]
    group_params = [{'params': decay_params, 'weight_decay': args.weight_decay}, {"params": non_decay_params}]
    opt = Momentum(params=group_params, learning_rate=Tensor(lr), momentum=args.momentum, weight_decay=args.weight_decay,
    ...
    for i, data in enumerate(data_loader):
        ...
        # save checkpoint, copy to s3
        if (i + 1) % args.ckpt_interval == 0 and args.rank_save_ckpt_flag:
            import glob
            ckpts = glob.glob(os.path.join(args.outputs_dir, '*.ckpt'))
            for ckpt in ckpts:
                ckpt_fn = os.path.basename(ckpt)
                if not ckpt_fn.startswith('{}-'.format(args.rank)):
                    continue
                if ckpt in ckpt_history:
                    continue
                ckpt_history.append(ckpt)
                copy_mox_ckpt_log_to_s3(args, ckpt)
            # shape_record.show(args.logger)
