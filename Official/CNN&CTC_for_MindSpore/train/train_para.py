...
from src.dataset import ST_MJ_Generator_batch_fixed_length_para
from src.CNNCTC.model import CNNCTC_Model, ctc_loss, WithLossCell, TrainOneStepCell_para
...
def dataset_creator():
    ds = GeneratorDataset(ST_MJ_Generator_batch_fixed_length_para, ['img', 'label_indices', 'text', 'sequence_length'],
                          num_parallel_workers=8)
    ds.set_dataset_size(int(TRAIN_DATASET_SIZE // (TRAIN_BATCH_SIZE * 8)))
...
def train(device_id):
    ...
    net = TrainOneStepCell_para(net, opt)
    ...
    if device_id == 0:
        ...
        model.train(TRAIN_EPOCHS, ds, callbacks=[callback], dataset_sink_mode=False)
