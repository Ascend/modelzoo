...
def uniform_recurisive_init(custom_cell):
    for name, cell in custom_cell.cells_and_names():
        if isinstance(cell, (nn.Conv2d, nn.Dense)):
            bound = 0.07
            cell.weight.default_input = Tensor(np.random.uniform(-bound, bound, cell.weight.default_input.shape()),
                                               cell.weight.default_input.dtype())
            if cell.bias is not None:
                cell.bias.default_input = Tensor(np.zeros(cell.bias.default_input.shape()),
                                                 cell.bias.default_input.dtype())
