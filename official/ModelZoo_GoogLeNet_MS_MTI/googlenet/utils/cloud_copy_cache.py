...
def copy_mox_ckpt_log_to_s3(args, ckpt):
    if args.train_url != '':
        import moxing as mox
        roma_weights_fp = os.path.join(args.train_url, ckpt)
        ...
        roma_log_fp = os.path.join(args.train_url, args.logger.log_fn)
        roma_log_dirname = os.path.dirname(roma_log_fp)
        if not mox.file.exists(roma_log_dirname):
            mox.file.make_dirs(roma_log_dirname)
        os.system(
            "python -c 'import moxing as mox; mox.file.copy(\"{}\", \"{}\")' &".format(args.logger.log_fn, roma_log_fp))
