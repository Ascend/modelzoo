...
def classification_dataset_c(data_dir, image_size, per_batch_size, max_epoch, rank, group_size, 
                      mode='train', 
                      input_mode='folder',
                      root='',
    ...
    num_parallel_workers1 = int(64 / group_size)
    if input_mode == 'folder':
        de_dataset = de.ImageFolderDatasetV2(data_dir, num_parallel_workers=num_parallel_workers1,
    ...
    de_dataset = de_dataset.map(input_columns="image", num_parallel_workers=num_parallel_workers1, operations=transform_img)
    # de_dataset = de_dataset.map(input_columns="label", num_parallel_workers=8, operations=transform_label)
 
    columns_to_project = ["image", "label"]
    de_dataset = de_dataset.project(columns=columns_to_project)
    
    num_parallel_workers2 = int(16 / group_size)
    de_dataset = de_dataset.batch(per_batch_size, num_parallel_workers=num_parallel_workers2, drop_remainder=drop_remainder)
