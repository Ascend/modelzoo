...
def parse_args():
    ...
    parser.add_argument("--mode", type=str, default="train",
                        help="trainning model or validate model")
...
def main():
    ...
    each_core = int(192 / args.nproc_per_node)
    for rank in range(0, args.nproc_per_node):
        rank_id = rank
        device_id = visible_devices[rank]
        if args.mode == 'train':
            device_root_fn = os.path.join(os.getcwd(), 'device{}'.format(rank_id))
        else:
            device_root_fn = os.path.join(os.getcwd(), 'eval_device{}'.format(rank_id))
        rank_process = ''
        if args.nproc_per_node > 1:
            rank_process += 'export RANK_TABLE_FILE={} && '.format(table_fn)
        begin_core = rank * each_core
        end_core = (rank + 1) * each_core - 1
        tie_core_command = 'taskset -c ' + str(begin_core) + '-' + str(end_core)
        rank_process += 'export RANK_SIZE={} && source {} && export RANK_ID={} && export DEVICE_ID={} && rm -rf {} && mkdir {} && cd {} && {} python {} '.format(args.nproc_per_node, args.env_sh, rank_id, device_id, device_root_fn, device_root_fn, device_root_fn, tie_core_command, args.training_script)
