...
from src.common.backbones.darknet import DarkNet, ResidualBlock
from src.object_detection.yolo_v3.config import Config_yolov3
from src.object_detection.losses.yolo_loss import XYLoss, WHLoss, ConfidenceLoss, ClassLoss
...
class DetectionBlock(nn.Cell):
    """
     ...
     Args:
         scale: Character.
         input_shape: The shape of raw input, default (416, 416)
    ...
    def __init__(self, scale, config=Config_yolov3(), is_training=True):
...
class Iou(nn.Cell):
    ...
    def construct(self, box1, box2):
        ...
        intersect_area = P.Squeeze(-1)(intersect_wh[:, :, :, :, :, 0:1]) * P.Squeeze(-1)(intersect_wh[:, :, :, :, :, 1:2]) # [batch, gx, gy, anchors, maxboxes]
...
class YoloLossBlock(nn.Cell):
    ...
    """
    def __init__(self, scale, config=Config_yolov3()):
        ...
        if scale == 's':
            ...
            idx = (3, 4, 5)
        elif scale == 'l':  # the feature is small, the view is large
            idx = (6, 7, 8)
        else:
            raise KeyError("Invalid scale value for DetectionBlock")
        self.anchors = Tensor([self.config.anchor_scales[i] for i in idx], ms.float32)
    ...
    def construct(self, grid, prediction, pred_xy, pred_wh, y_true, gt_box, input_shape):
        ...
        class_probs = y_true[:, :, :, :, 5:]    # cls
        # true_boxes = y_true[:, :, :, :, :4]     # xywh

        grid_shape = P.Shape()(prediction)[1:3]     # gridy, gridx
        grid_shape = P.Cast()(F.tuple_to_array(grid_shape[::-1]), ms.float32) # gridx, gridy

        pred_boxes = self.concat((pred_xy, pred_wh))    # xywh
        true_xy = y_true[:, :, :, :, :2] * grid_shape - grid
        true_wh = y_true[:, :, :, :, 2:4]
        true_wh = P.Select()(P.Equal()(true_wh, 0.0),
                             P.Fill()(P.DType()(true_wh),  # same size as true_wh fill with 1
                                      P.Shape()(true_wh), 1.0),
                             true_wh)
...
class yolov3_darknet53(nn.Cell):
    """
    ...
    def __init__(self, is_training):
        super(yolov3_darknet53, self).__init__()
        self.config = Config_yolov3()
...
class YoloWithLossCell(nn.Cell):
    def __init__(self, network):
        ...
        # self.config = ConfigYOLOV3ResNet18()
        self.config = Config_yolov3()
