...
class SessionImpl:
	...
	def create_eval_ops(self, preprocessing_name):
		...
		eval_accuracy, metric_update_op = tf.metrics.accuracy(tf.argmax(labels_eval, 1), predictions)
		tf.summary.scalar('metric_update_op', metric_update_op)
	...
	def train(self, train_op, summary_op, total_loss, global_step, iterator_train=None,
		...
		with tf.Session(config=config) as sess:
			...
			if self.env.FLAGS.checkpoint_path == '':
				...
				if tf.gfile.IsDirectory(self.env.FLAGS.checkpoint_path):
	...
	def create_data(self, dataset, preprocessing_name, is_training):
		if not self.env.FLAGS.fake_input:
			...
			tf.summary.image('input images', images, max_outputs=4)
