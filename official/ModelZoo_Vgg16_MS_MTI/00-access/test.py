...
def parse_args(cloud_args={}):
    ...
    # network related
    parser.add_argument('--backbone', default='vgg16', help='backbone')
...
def test(cloud_args={}):
    ...
    for model in args.models:
        de_dataset = classification_dataset_c(args.data_dir, image_size=args.image_size,
                                      per_batch_size=args.per_batch_size,
                                      max_epoch=1, rank=args.rank, group_size=args.group_size,
                                      mode='eval')
        eval_dataloader = de_dataset.create_tuple_iterator()
        network = get_network(args.backbone, args.num_classes, 'test')
        ...
        if args.rank == 0:
            ...
            args.logger.info('Inference Performance: {:.2f} imgs/sec'.format(fps))
...
if __name__ == "__main__":
    """
    Test script. Get top1 and top5 ACC from classfication dataset.
