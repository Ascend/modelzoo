...
def resnet_main(
  ...
  session_config.gpu_options.allocator_type = 'BFC'  # 使用BFC算法
  ...
  if use_train_and_evaluate:
    ...
    if train_epochs == 0:
      ...
      # for final epoch train_accuracy
      if schedule[-1] > 1:
        schedule[-1] = schedule[-1] -1
        schedule.append(1)

      logging.info('schedule: %s', ','.join([str(i) for i in schedule]))

    for cycle_index, num_train_epochs in enumerate(schedule):
      logging.info('Starting cycle: %d/%d Num_train_epochs: %d', cycle_index, int(n_loops), int(num_train_epochs))
