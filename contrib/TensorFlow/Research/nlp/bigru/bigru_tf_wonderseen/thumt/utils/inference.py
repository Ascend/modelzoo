...
class BeamSearchState(namedtuple("BeamSearchState", ("inputs", "state", "finish"))):
    pass
...
def _get_inference_fn(model_fns, features):
    def inference_fn(inputs, state):
        local_features = {
            ...
            "target_length": tf.fill([EVAL_BATCH_SIZE], EVAL_DECODE_LENGTH)
        }
...
def _npu_inference_fn(model_fns, inputs, state):
    local_features = {
        "source": inputs["source"],
        "source_length": inputs["source_length"],
        # [bos_id, ...] => [..., 0]
        "target": tf.fill([EVAL_BATCH_SIZE, 1], BOS_ID),
        # "target_length": tf.fill([tf.shape(inputs)[0]], tf.shape(inputs)[1])
        "target_length": tf.fill([EVAL_BATCH_SIZE], EVAL_DECODE_LENGTH)
    }
    outputs = []
    ...
    # Ensemble
    log_prob = tf.add_n(outputs) / float(len(outputs))
    log_prob = tf.argmax(log_prob, axis=-1)
...
def create_inference_graph(models, features, params):
    ...
    # # batch_size = features["source"].shape[0] # dymanic
    # batch_size = EVAL_BATCH_SIZE # static shape
    ...
    # # features["source"] = tf.reshape(features["source"], [-1, shape[2]])
    # features["source"] = tf.reshape(features["source"], [batch_size * beam_size, SEQ_LENGTH])
    ...
    # # shape = features["source_length"].shape
    # max_length = features["source_length"] + decode_length
    # # max_length = decode_length
    # # [batch, beam_size, length] => [batch * beam_size, length]
    # features["source_length"] = tf.reshape(features["source_length"], [batch_size * beam_size])
    ...
    ## NPU version
    seqs, scores = _npu_inference_fn(funcs, features, states)
    return seqs, scores
