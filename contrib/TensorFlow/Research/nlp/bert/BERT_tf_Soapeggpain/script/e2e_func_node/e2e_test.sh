...
{   
    ...
    do
        ...
        do
	        ...
	        RUN_ALGORITHM_CMD="${currentDir}${RUN_ALGORITHM_CMD}"
            fi
            ...
            if [ x"${mode}" = xhost ];
            then
                CHECKPOINT_DIR=`echo ${CHECKPOINT_DIR/'--model_dir='/'--model_dir='${currentDir}'/'}`
                echo 'new checkpoint '${CHECKPOINT_DIR}''
...
function docker_run()
{
    ...
    if [ x = x"$delckpt" ]; then
        echo "[`date +%Y%m%d-%H:%M:%S`] [INFO] delckpt is NULL"
    else
        echo "[`date +%Y%m%d-%H:%M:%S`] [INFO] delete ckpt"
        find ${exec_path} -name "*.ckpt*" | xargs rm -rf
    fi
}
...
function host_run()
{
    ...
    # echo "PYTHONPATH=$PYTHONPATH:${currentDir}:${currentDir}/code:${currentDir}/code/resnet50_train" >> ${exec_path}/npu_set_env.sh

    # sed -i 's#{DATA_URL}#file://'${currentDir}'/data#g'    `grep DATA_URL -rl "${currentDir}/code/resnet50_train/configs/"`
    # sed -i 's#{LOG_DIR}#'${currentDir}'/d_solution/ckpt0#g'     `grep LOG_DIR -rl "${currentDir}/code/resnet50_train/configs/"`
...
function result_check()
{
    ...
    if [ x = x"$generate_flag" ]; then
        echo "[`date +%Y%m%d-%H:%M:%S`] [INFO] generate_flag is NULL"
    else
        python ${currentDir}/tools/createreport.py ${generate_flag} ${RANK_ID} 10.90.16.178 huawei@12#$
    fi
}
...
if [ x"${timeout}" = x ] ;
then
    timeout=1209600 # default=1209600 seconds
fi
