...
def get_subwindow_tracking(im, pos, model_sz, original_sz, avg_chans, out_mode='tensorflow'):
    # out_mode = 'tensorflow' or 'torch'
...
def siamese_init(im, target_pos, target_sz, model, hp=None, device='cpu'):
    ...
    z_crop = get_subwindow_tracking(im, target_pos, p.exemplar_size, s_z, avg_chans)[np.newaxis, ...]
    ...
    state['p'] = p
    state['start_frame_image'] = z_crop
    state['net'] = model
...
def siamese_track(state, im, mask_enable=False, refine_enable=False, device='cpu', debug=False):
    p = state['p']
    net = state['net']
    start_frame_image = state['start_frame_image']
    ...
    x_crop = get_subwindow_tracking(im, target_pos, p.instance_size, round(s_x), avg_chans)[np.newaxis, ...]
    # print("template")
    # print(start_frame_image.shape)
    # print(x_crop.shape)
    train_feed_dict = {net["input"]["template"]: start_frame_image,
                       net["input"]["search"]: x_crop}

    delta = None
    if mask_enable:
        #         print(os.path.join(os.path.abspath(os.path.join(os.getcwd(), ".")), "tmp/template.jpg"))
        cv2.imwrite(os.path.join(os.path.abspath(os.path.join(os.getcwd(), ".")), "tmp/template.jpg"),
                    start_frame_image[0])

        cv2.imwrite(os.path.join(os.path.abspath(os.path.join(os.getcwd(), ".")), "tmp/search.jpg"), x_crop[0])

        os.system("bash convert_image2bin_inference.sh")

        output_path = os.listdir(os.path.abspath(os.path.join(os.getcwd(), "tmp/output1/")))[0]
        score = np.fromfile(os.path.join(os.path.abspath(os.path.join(os.getcwd(), ".")),
                                         "tmp/output1/{}/siammask_output_0.bin".format(output_path)), dtype='float32')
        delta = np.fromfile(os.path.join(os.path.abspath(os.path.join(os.getcwd(), ".")),
                                         "tmp/output1/{}/siammask_output_1.bin").format(output_path), dtype='float32')
        mask = np.fromfile(os.path.join(os.path.abspath(os.path.join(os.getcwd(), ".")),
                                        "tmp/output1/{}/siammask_output_2.bin").format(output_path), dtype='float32')
        score = np.reshape(score, (1, 25, 25, 10))
        delta = np.reshape(delta, (1, 25, 25, 20))
        mask = np.reshape(mask, (1, 25, 25, 3969))

        #         [rpn_pred_cls, rpn_pred_loc, rpn_pred_mask, template_feature, search_feature] = net["pre"]
        #         score, delta, mask = net["sess"].run(
        #             [rpn_pred_cls, rpn_pred_loc, rpn_pred_mask], feed_dict=train_feed_dict)

        score = np.transpose(score, (0, 3, 1, 2))
    ...
    def softmax(x):
        ...
        return np.exp(x) / (np.sum(np.exp(x), axis=0) + 1e-10)

    delta = np.reshape(np.transpose(delta, (1, 2, 3, 0)), (4, -1))
    # print(delta.shape)
    score = np.transpose(np.reshape(np.transpose(score, (1, 2, 3, 0)), (2, -1)), (1, 0))
    for i in range(len(score)):
        score[i] = softmax(score[i])
    score = score[:, 1]
    ...
    if mask_enable:
        ...
        if refine_enable:
            ...
            # print("mask.shape")
            # print(mask.shape)
            mask = np.reshape(np.squeeze(sigmoid_fun(mask[0, :, delta_y, delta_x])), (p.out_size, p.out_size))
