...
def bottleneck(inputs,
    ...
    with tf.variable_scope(scope, 'bottleneck_v1', [inputs]) as sc:
        if rate == 2:
            downsample_padding = "VALID"
        depth_in = slim.utils.last_dimension(inputs.get_shape(), min_rank=4)
        if depth == depth_in:
            shortcut = resnet_utils.subsample(inputs, stride, 'shortcut', padding=downsample_padding,rate=rate,depth=depth)
        else:
            shortcut = slim.conv2d(
                ...
                scope='shortcut', padding=downsample_padding)

        residual = slim.conv2d(inputs, depth_bottleneck, [1, 1], stride=1,
                               scope='conv1', padding='VALID', biases_initializer=None)
        residual = slim.conv2d(residual, depth_bottleneck, [3, 3], stride=stride,
                               rate=rate, scope='conv2', padding=padding, biases_initializer=None)
        # residual = resnet_utils.conv2d_same(residual, depth_bottleneck, 3, stride,
        #                                     rate=rate, scope='conv2', padding=padding)
        residual = slim.conv2d(residual, depth, [1, 1], stride=1, biases_initializer=None,
                               activation_fn=None, scope='conv3', padding='VALID')
            ...
            if is_training is not None else NoOpScope()):
                ...
                if include_root_block:
                    if output_stride is not None:
                        ...
                        output_stride /= 4
                    # net = resnet_utils.conv2d_same(net, 64, 7, stride=2, scope='conv1', padding='VALID')
...
def resnet_v1_block(scope, base_depth, num_units, stride, rate=1, padding='VALID',
                    downsample_padding='SAME'):
...
def resnet_v1_50(inputs,
    ...
    blocks = [
        resnet_v1_block('block1', base_depth=depth_func(64), num_units=3,
                        stride=1, padding='SAME', downsample_padding='VALID'),
        resnet_v1_block('block2', base_depth=depth_func(128), num_units=4,
                        stride=2, padding='VALID', downsample_padding='VALID'),
        resnet_v1_block('block3', base_depth=depth_func(256), num_units=6,
                        stride=1, rate=2, padding='SAME', downsample_padding='SAME'),
