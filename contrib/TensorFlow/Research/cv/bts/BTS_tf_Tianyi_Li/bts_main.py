...
def train(params):
    #with tf.Graph().as_default(), tf.device('/cpu:0'):#original code
    with tf.Graph().as_default():
        ...
        with tf.variable_scope(tf.get_variable_scope()):
            for i in range(args.num_gpus):
                ...
                #with tf.device('/gpu:%d' % i):
                image, depth_gt, focal = dataloader_iter.get_next()
                model = BtsModel(params, args.mode, image, depth_gt, focal=focal, reuse_variables=reuse_variables, model_index=i, bn_training=False)
