...
_RAW_SEMANTIC_PREDICTION_SAVE_FOLDER = 'results/VOC2012/Segmentation/comp6_test_cls/'
...
def _process_batch(sess, original_images, semantic_predictions, image_names,
  ...
  for i in range(num_image):
    ...
    save_annotation.save_annotation(
        ...
        image_names[i].decode(), add_colormap=True,
    ...
    if FLAGS.also_save_raw_predictions:
      ...
      if train_id_to_eval_id is not None:
        crop_semantic_prediction = _convert_train_id_to_eval_id(
            ...
            train_id_to_eval_id)
      save_annotation.save_annotation(
          crop_semantic_prediction, raw_save_dir, image_filename.decode(),
          add_colormap=False)
  ...
  with tf.Graph().as_default():
    ...
    for checkpoint_path in checkpoints_iterator:
      ...
      session_creator = tf.train.ChiefSessionCreator(
          scaffold=scaffold,
          master=FLAGS.master,
          checkpoint_filename_with_path=checkpoint_path,config=session_config)
