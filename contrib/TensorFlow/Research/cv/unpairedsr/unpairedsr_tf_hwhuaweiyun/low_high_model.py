...
class MODEL(object):
    ...
    def __init__(self, config):
        self.config = config
        self.lr_img = tf.placeholder(tf.float32, [None, 16, 16, 3])
        self.hr_img = tf.placeholder(tf.float32, [None, 64, 64, 3])
        ...
        with tf.variable_scope('generator', reuse=tf.AUTO_REUSE):
            ...
            for ru in range(len(self.res_units)):
                ...
                if ru < 2:
                    ...
                    out = tf.nn.relu(out)
                    # out = tf.layers.conv2d_transpose(out, filters=nunits, kernel_size=4, strides=2)
