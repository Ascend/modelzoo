...
class BaseSummaries(ABC):
    ...
    def __init__(self, session, train_dir, is_training,
                 max_image_outputs=None):
    ...
    def setup_experiment(self, config):
        ...
        self.summary_writer.add_graph(tf.get_default_graph())  # This is magic
    ...
    def _build_summary_directory(self, is_training):
        if is_training:
            ...
            if Path(self.train_dir).is_dir():
                summary_directory = (Path(self.train_dir) / Path("eval")).as_posix()
            else:
                summary_directory = (Path(self.train_dir).parent / Path("eval")).as_posix()
    ...
    def _routine_add_summary_op(self, summary_value_type, summary_key, value, collection_key_suffix):
        ...
        elif summary_value_type == self.VALUE_TYPES.IMAGE:
            def register_fn(k, v):
                return tf.summary.image(k, v, max_outputs=self.max_image_outputs, collections=[collection_key])

        elif summary_value_type == self.VALUE_TYPES.HISTOGRAM:
            def register_fn(k, v):
                return tf.summary.histogram(k, v, collections=[collection_key])
    ...
    # since it has bug with @overload
    # @classmethod
    @overload
