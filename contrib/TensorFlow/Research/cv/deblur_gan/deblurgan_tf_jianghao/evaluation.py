...
def calculate_ssim(img1, img2, crop_border, test_y_channel=False):
    assert img1.shape == img2.shape, (
        f'Image shapes are differnet: {img1.shape}, {img2.shape}.')
...
def calc_measures(args, calc_psnr=True, calc_ssim=True):
    ...
    """
    sharp_img_name = sorted(os.listdir(args.HR_data_dir))
    infer_img_name = sorted(os.listdir(args.inference_result))
    # HR_files = glob.glob(args.HR_data_dir + '/*')
    mean_psnr = []
    mean_ssim = []
    test_y_channel = False

    for i, file in enumerate(sharp_img_name):
        hr_img = cv2.imread(os.path.join(args.HR_data_dir, file)).astype(
            np.float32) / 255
        ...
        inf_img = cv2.imread(
            os.path.join(args.inference_result, infer_img_name[i])).astype(
                np.float32) / 255
        ...
        print('-' * 10)
        if calc_psnr:
            # psnr = compare_psnr(hr_img * 255, inf_img * 255)
            psnr = calculate_psnr(hr_img * 255, inf_img * 255,
                                  args.crop_border, test_y_channel)
            print('{0} : PSNR {1:.3f} dB'.format(filename, psnr))
            mean_psnr.append(psnr)
        if calc_ssim:
            # ssim = compare_ssim(hr_img * 255, inf_img * 255, multichannel=True)
