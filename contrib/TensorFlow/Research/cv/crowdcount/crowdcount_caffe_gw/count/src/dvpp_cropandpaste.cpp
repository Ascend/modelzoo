...
{
    destroy_crop_and_paste_resource();
}

Result DvppCropAndPaste::init_crop_and_paste_input_desc(ImageData& inputImage)
{
...
Result DvppCropAndPaste::init_crop_and_paste_output_desc()
{
...
// IN/OUT Desc
Result DvppCropAndPaste::init_crop_and_paste_resource(ImageData& inputImage) {
    format_ = static_cast<acldvppPixelFormat>(PIXEL_FORMAT_YUV_SEMIPLANAR_420);
    if (SUCCESS != init_crop_and_paste_input_desc(inputImage)) {
        ERROR_LOG("InitCropAndPasteInputDesc failed");
        return FAILED;
    }

    if (SUCCESS != init_crop_and_paste_output_desc()) {
...
{
    if (SUCCESS != init_crop_and_paste_resource(srcImage)) {
    ...
    // must odd
    uint32_t pasteRightOffset = CHECK_ODD(size_.width - 2 * dx);
    // must odd
    uint32_t pasteBottomOffset = CHECK_ODD(size_.height -  2 * dy);
...
void DvppCropAndPaste::destroy_crop_and_paste_resource()
{
    if (cropArea_ != nullptr) {
