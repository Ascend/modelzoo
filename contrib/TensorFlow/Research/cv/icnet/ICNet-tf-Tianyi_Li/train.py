...
def get_mask(gt, num_classes, ignore_label):
    ...
    #print("测试 indices：", indices)
    
    with npu_scope.without_npu_compile_scope():
        temp_ = tf.where(mask)
    indices = tf.squeeze(temp_, 1)
...
def create_loss(output, label, num_classes, ignore_label):
    raw_pred = tf.reshape(output, [-1, num_classes]) # yuanlai code
    '''
    ...
    label = tf.reshape(label, [-1,]) # yuanlai code
    '''
    global i_list
    global i_count
    ...
    loss = tf.nn.sparse_softmax_cross_entropy_with_logits(logits=pred, labels=gt) # yuanlai code
    reduced_loss = tf.reduce_mean(loss) # yuanlai code
...
def main():
    ...
    train_reader = ImageReader(cfg=cfg, mode='train') # yuanlai code
    ...
    with compat.forward_compatibility_horizon(2019, 5, 1):
        ...
        print("test run with compat.forward_compatibility_horizon(2019, 5, 1):")
    ...
    if args.update_mean_var == False:
        print("test set update_mean_var == False")
        update_ops = None
    else:
        update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
        print("test set update_mean_var == False wrong")


    print("we know set update_ops：", update_ops)
