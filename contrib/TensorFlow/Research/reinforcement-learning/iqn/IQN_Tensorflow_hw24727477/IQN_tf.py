...
import os
os.system("pip3 install atari-py")
os.system("pip3 install gym==0.10.9")
...
parser.add_argument('--games', type=str, default="Boxing", help='name of the games. for example: Breakout')
parser.add_argument('--data_url', default=None, type=str, help='input_dir')
parser.add_argument('--train_url', default=None, type=str, help='output_dir')
...
PRED_PATH = './iqn_pred_net_' + args.games
TARGET_PATH = './iqn_target_net_' + args.games
RESULT_PATH = './result'

#OBS_OUT_PATH = 's3:///model-train-fzk/output/'
OBS_OUT_PATH = args.train_url
OBS_PRED_PATH = os.path.join(OBS_OUT_PATH, os.path.basename(PRED_PATH))
OBS_TARGET_PATH = os.path.join(OBS_OUT_PATH, os.path.basename(TARGET_PATH))
OBS_RESULT_PATH = os.path.join(OBS_OUT_PATH, os.path.basename(RESULT_PATH))
...
class DQN(object):
    def __init__(self):
        ...
        #self.choose_net = ConvNet("choose_net", N_ENVS)
        self.IQN_graph()
        ...
        # for choose_parm, pred_param in zip(self.choose_net.parameters(), self.pred_net.parameters()):
        #     self.update_choose_op.append(tf.assign(choose_parm, pred_param))
    ...
    def choose_action(self, x, EPSILON):
        ...
        if np.random.uniform() >= EPSILON:
            ...
            # action_value = np.mean(action_value, axis=2)
    ...
    def learn(self):
        ...
        # b_a = np.ones(shape=[BATCH_SIZE], dtype=np.int32)
        # b_s = np.ones(shape=[BATCH_SIZE, 4, 84, 84], dtype=np.float32)
        # b_s_ = np.ones(shape=[BATCH_SIZE, 4, 84, 84], dtype=np.float32)
        # b_d = np.zeros(shape=[BATCH_SIZE], dtype=np.float32)
        # b_r = np.ones(shape=[BATCH_SIZE], dtype=np.float32)
        # b_w, b_idxes = np.ones_like(b_r), None
...
def train(epsilon, model, restart_step=0):
    ...
    for step in range(restart_step + 1, STEP_NUM // N_ENVS + 1):
...
if args.mode == 'eval' or LOAD:
    mox.file.copy_parallel(OBS_PRED_PATH, PRED_PATH)
    mox.file.copy_parallel(OBS_TARGET_PATH, TARGET_PATH)
    mox.file.copy_parallel(OBS_RESULT_PATH, RESULT_PATH)
