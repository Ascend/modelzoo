...
class dataloader():
    ...
    def build_loader(self):
        if self.mode == 'train':
            ...
            iterator = tf.data.Iterator.from_structure(self.tr_dataset.output_types,
                self.tr_dataset.output_shapes)
            ...
            iterator = tf.data.Iterator.from_structure(self.val_dataset.output_types,
            ...
            iterator = tf.data.Iterator.from_structure(self.te_dataset.output_types,
                self.te_dataset.output_shapes)
            self.next_batch = iterator.get_next()
            self.init_op = {}
    ...
    def _resize(self, image_blur, image_sharp):
        ...
        image_blur = tf.image.resize_images(image_blur, (self.load_Y, self.load_X),
            tf.image.ResizeMethod.BICUBIC)
        image_sharp = tf.image.resize_images(image_sharp, (self.load_Y, self.load_X),
            tf.image.ResizeMethod.BICUBIC)
