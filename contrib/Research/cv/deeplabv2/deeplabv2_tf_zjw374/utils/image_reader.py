...
def image_scaling(img, label):
    ...
    scale = [0.85] #tf.random_uniform([1], minval=0.5, maxval=1.5, dtype=tf.float32, seed=None)
...
def random_crop_and_pad_image_and_labels(image, label, crop_h, crop_w, ignore_label=255):
    ...
    print("###############", tf.shape(combined)[0], tf.shape(combined)[1], tf.shape(combined)[2])
    image_shape = tf.shape(image)
    #combined_pad = tf.image.pad_to_bounding_box(combined, 0, 0, crop_h + 10, crop_w + 10)
    #combined_pad = tf.image.pad_to_bounding_box(combined, 0, 0, tf.maximum(crop_h, image_shape[0]), tf.maximum(crop_w, image_shape[1]))
    combined_pad = tf.image.resize_images(combined, [crop_h, crop_w])  # ####
    last_image_dim = tf.shape(image)[-1]
    # last_label_dim = tf.shape(label)[-1]
    print("***************", tf.shape(combined_pad)[0], tf.shape(combined_pad)[1], tf.shape(combined_pad)[2])
    #combined_crop = tf.random_crop(combined_pad, [crop_h, crop_w, 4])
    print("------------------------------")
    a, b, c, d = tf.split(axis = 2, num_or_size_splits = 4, value = combined_pad)
    img_crop = tf.concat(axis=2, values=[a, b, c])
    label_crop = d
    #img_crop = combined_crop[:crop_h, :crop_h, :last_image_dim]
