...
if __name__ == '__main__':
    ...
    start = datetime.datetime.now()
    os.system('tar -xf {} -C {}'.format('/cache/dataset.tar', '/cache'))
    end = datetime.datetime.now()
    print("===>>>Uncompressing finished: time use:{}(s)".format((end - start).seconds))

    # run training
    print("===>>>Begin training:")
    os.system('cd /home/work/user-job-dir/code && sh /home/work/user-job-dir/code/scripts/train_mmnet_dm1.0_256.sh /cache/dataset /cache/experiments')
    print("===>>>Training finished:")

    #  copy results from local to obs
    local_dir = '/cache/experiments'
    remote_dir = os.path.join(config.train_url, 'experiments')
