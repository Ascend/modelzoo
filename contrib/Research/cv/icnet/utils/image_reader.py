...
def _random_crop_and_pad_image_and_labels(image, label, crop_h, crop_w, ignore_label):
    ...
    ############################# ##############################
    combined_crop = tf.random_crop(combined_pad, [crop_h, crop_w, 4]) # 原始代码
    #combined_crop = tf.random_crop(combined_pad, [720, 720, 4])
    #################################################################
    ...
    # img_crop.set_shape((720, 720, 3))
    # label_crop.set_shape((720, 720, 1))
    ########################################
...
class ImageReader(object):
    ...
    def create_tf_dataset(self, cfg):
        ...
        if cfg.is_training: # Training phase
            ...
            # dataset = dataset.map(lambda x, y: temp_li(x, y), num_parallel_calls=cfg.N_WORKERS)
            ####################################################################################33
