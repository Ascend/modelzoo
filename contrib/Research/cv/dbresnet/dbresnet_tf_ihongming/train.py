...
def main():
    ...
    input_images = tf.placeholder(tf.float32, shape=[batch_size, img_size, img_size, 3], name='input_images')
    input_score_maps = tf.placeholder(tf.float32, shape=[batch_size, img_size, img_size, 1], name='input_score_maps')
    input_threshold_maps = tf.placeholder(tf.float32, shape=[batch_size, img_size, img_size, 1], name='input_threshold_maps')
    input_score_masks = tf.placeholder(tf.float32, shape=[batch_size, img_size, img_size, 1], name='input_score_masks')
    input_threshold_masks = tf.placeholder(tf.float32, shape=[batch_size, img_size, img_size, 1], name='input_threshold_masks')

    global_step = tf.get_variable('global_step', [], initializer=tf.constant_initializer(0), trainable=False)
    learning_rate = tf.train.exponential_decay(cfg.TRAIN.LEARNING_RATE, global_step, decay_steps=2000,decay_rate=0.96, staircase=True)
    ...
    avg_binarize_acc = binarize_acc
    avg_thresh_binary_acc = thresh_binary_acc
...
#     config.gpu_options.allow_growth = True
    ...
    with tf.Session(config=config) as sess:
        ...
        val_data_generator = generate()
        test_data_generator = generate(is_training=False)
        test_epoch = 0
        ...
        for step in range(cfg.TRAIN.MAX_STEPS):
            ...
            if step % cfg.TRAIN.SAVE_SUMMARY_STEPS == 0:
                ...
                val_feed_dict = {input_images: val_data[0],
                                 input_score_maps: val_data[1][..., np.newaxis],
                                 input_threshold_maps: val_data[3][..., np.newaxis],
                                 input_score_masks: val_data[2][..., np.newaxis],
                                 input_threshold_masks: val_data[4][..., np.newaxis]}
            ...
            if step % cfg.EVAL.TEST_STEP == 0 and step != 0:
                ...
                while temp_epoch == test_epoch:
                    ...
                    test_feed_dict = {input_images: test_data[0],
                                      input_score_maps: test_data[1][..., np.newaxis],
                                      input_threshold_maps: test_data[3][..., np.newaxis],
                                      input_score_masks: test_data[2][..., np.newaxis],
                                      input_threshold_masks: test_data[4][..., np.newaxis]}
                    test_epoch += cfg.EVAL.TEST_STEP
...
if __name__ == '__main__':
    ...
    parser.add_argument('--max_steps', '-m', default=100000, type=int, help='max_steps integer')
    parser.add_argument('--save_steps', '-s', default=2000, type=int, help='max_steps integer')
