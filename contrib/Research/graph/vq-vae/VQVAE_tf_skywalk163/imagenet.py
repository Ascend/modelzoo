...
def main(config,
    ...
    with tf.variable_scope('train'):
        ...
        with tf.variable_scope('params') as params:
            pass
        net = VQVAE(learning_rate, global_step, BETA, train_ims,
                    K, D, _imagenet_arch, params, True)

    with tf.variable_scope('valid'):
        params.reuse_variables()
        valid_net = VQVAE(None, None, BETA, valid_ims, K,
                          D, _imagenet_arch, params, False)
...
def train_prior(config,
    ...
    with tf.variable_scope('net'):
        with tf.variable_scope('params') as params:
            pass
        vq_net = VQVAE(None, None, BETA, ims, K, D,
                       _imagenet_arch, params, False)
    # <<<<<<<
