...
def _cifar10_arch(d):
    ...
    with tf.variable_scope('enc') as enc_param_scope:
        enc_spec = [
            ...
            lambda t, **kwargs: tf.nn.relu(t),
            partial(_residual,
                    conv3=Conv2d('res_1_3', d, d, 3, 3, 1,
                                 1, data_format='NHWC'),
                    conv1=Conv2d('res_1_1', d, d, 1, 1, 1, 1, data_format='NHWC')),
            partial(_residual,
                    conv3=Conv2d('res_2_3', d, d, 3, 3, 1,
                                 1, data_format='NHWC'),
...
def _imagenet_arch(d, num_residual=4):
    ...
    with tf.variable_scope('enc') as enc_param_scope:
        ...
        enc_spec += [
            partial(_residual,
                    conv3=Conv2d('res_%d_3' % i, d, d, 3, 3,
                                 1, 1, data_format='NHWC'),
...
class VQVAE():
    def __init__(self, lr, global_step, beta,
        ...
        with tf.variable_scope('forward') as forward_scope:
            ...
            self.nll = -1.*(tf.reduce_mean(tf.log(self.p_x_z),
                                           axis=[1, 2, 3]) + tf.log(1/tf.cast(K, tf.float32)))/tf.log(2.)
        ...
        if(is_training):
            with tf.variable_scope('backward'):
                ...
                # Embedding Grads
                embed_grads = list(
                    zip(tf.gradients(self.vq, embeds), [embeds]))

                optimizer = tf.train.AdamOptimizer(lr)
                self.train_op = optimizer.apply_gradients(
                    decoder_grads+encoder_grads+embed_grads, global_step=global_step)
        else:
    ...
    def __init__(self, lr, global_step, grad_clip,
        ...
        for i in range(num_layers):
            ...
            with tf.variable_scope("v_stack"+i):
                v_stack = GatedCNN([filter_size, filter_size, num_maps],
                                   v_stack_in, mask=mask, conditional=onehot_h).output()
                v_stack_in = v_stack

            with tf.variable_scope("v_stack_1"+i):
                v_stack_1 = GatedCNN(
                    [1, 1, num_maps], v_stack_in, gated=False, mask=mask).output()

            with tf.variable_scope("h_stack"+i):
                h_stack = GatedCNN([1, filter_size, num_maps], h_stack_in,
                                   payload=v_stack_1, mask=mask, conditional=onehot_h).output()

            with tf.variable_scope("h_stack_1"+i):
                h_stack_1 = GatedCNN(
                    [1, 1, num_maps], h_stack, gated=False, mask=mask).output()
                if residual:
                    h_stack_1 += h_stack_in  # Residual connection
                h_stack_in = h_stack_1

        with tf.variable_scope("fc_1"):
            fc1 = GatedCNN([1, 1, num_maps], h_stack_in,
                           gated=False, mask='b').output()

        with tf.variable_scope("fc_2"):
            self.fc2 = GatedCNN([1, 1, K], fc1, gated=False,
        ...
        save_vars = tf.get_collection(
            tf.GraphKeys.TRAINABLE_VARIABLES, tf.contrib.framework.get_name_scope())
        self.saver = tf.train.Saver(var_list=save_vars, max_to_keep=3)
        ...
        if(is_training):
            with tf.variable_scope('backward'):
                ...
                gradients = optimizer.compute_gradients(
                    self.loss, var_list=save_vars)
                if(grad_clip is None):
                    clipped_gradients = gradients
                else:
                    clipped_gradients = [
                        (tf.clip_by_value(_[0], -grad_clip, grad_clip), _[1]) for _ in gradients]
                    #clipped_gradients = [(tf.clip_by_average_norm(_[0], grad_clip), _[1]) for _ in gradients]
                self.train_op = optimizer.apply_gradients(
                    clipped_gradients, global_step)
    ...
    def sample_from_prior(self, sess, classes, batch_size):
        ...
        for i in xrange(size):
            for j in xrange(size):
                sampled, log_prob = sess.run(
                    [self.sampled, self.log_prob], feed_dict=feed_dict)
