...
def main(config,
    ...
    with tf.variable_scope('train'):
        ...
        with tf.variable_scope('params') as params:
            pass
        net = VQVAE(learning_rate, global_step, BETA,
                    resized, K, D, _mnist_arch, params, True)

    with tf.variable_scope('valid'):
        params.reuse_variables()
        valid_net = VQVAE(None, None, BETA, resized, K,
                          D, _mnist_arch, params, False)
    ...
    for step in tqdm(xrange(TRAIN_NUM), dynamic_ncols=True):
        ...
        it, loss, _ = sess.run(
            [global_step, net.loss, net.train_op], feed_dict={x: batch_xs})
...
def extract_z(MODEL,
    ...
    with tf.variable_scope('net'):
        with tf.variable_scope('params') as params:
            pass
        net = VQVAE(None, None, BETA, resized, K,
                    D, _mnist_arch, params, False)
...
def train_prior(config,
                ...
                SAVE_PERIOD,
                **kwargs):
    # for error ValueError: Variable net/params/enc/conv2d_1/w already exists, disallowed.
    ...
    with tf.variable_scope('misc'):
        ...
        sample_summary_op = tf.summary.image(
            'samples', sample_images, max_outputs=20)
...
def get_default_param():
    ...
    return {
        ...
        'TRAIN_NUM': 100,  # Size corresponds to one epoch
        'BATCH_SIZE': 32,  # 32

        'LEARNING_RATE': 0.0002,
        'DECAY_VAL': 1.0,
        'DECAY_STEPS': 200,  # Half of the training procedure.
