...
def _evaluate(eval_fn, input_fn, decode_fn, path, config, device_list):
    ...
    with graph.as_default():
        ...
        for i in range(len(device_list)):
            ...
            # })
            placeholders.append({
                "source": tf.placeholder(tf.int32, [None, EVAL_ENCODE_LENGTH], "source_%d" % i),
                "source_length": tf.placeholder(tf.int32, [None], "source_length_%d" % i)
            })

        predictions = [eval_fn(p) for p in placeholders]

        # predictions = [pred[0][:, 0, :] for pred in predictions] # original
        predictions = [pred[0][:, 0, :] for pred in predictions]

        # all_refs = [[] for _ in range(len(refs))]
        all_refs = [[]] # only one reference
        all_outputs = []
        ...
        with tf.train.MonitoredSession(session_creator=sess_creator) as sess:
            while not sess.should_stop():
                ...
                # input(inp_feats['source'])

                op, feed_dict = _shard_features(inp_feats, placeholders, predictions)
                ...
                feats["references"] = feats["references"][:,0,:]
                references = [item.tolist() for item in feats["references"]]
                all_refs[0].extend(references)
        ...
        decoded_symbols_sentence = [' '.join(m) for m in decoded_symbols]
        decoded_refs_sentence = [' '.join(m[0]) for m in decoded_refs]   
        with open('reference', 'w') as f:
            for m in decoded_refs_sentence: f.write(m+'\n')
        with open('prediction', 'w') as f:
            for m in decoded_symbols_sentence: f.write(m+'\n')

        print([(a, b) for a,b in zip(decoded_symbols_sentence, decoded_refs_sentence)])
...
class EvaluationHook(tf.train.SessionRunHook):
    ...
    def begin(self):
        ...
        if not tf.gfile.Exists(self._save_path):
            print("Making dir: %s" % self._save_path)
    ...
    def after_run(self, run_context, run_values):
        ...
        if self._timer.should_trigger_for_step(stale_global_step):
            ...
            if self._timer.should_trigger_for_step(global_step):
                ...
                #saver = _get_saver()
                #print("Saving checkpoints for %d into %s." %
