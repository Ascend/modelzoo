...
#define CHECK_NULL_RET(msg, ret)                                                  \
    if(ret == nullptr) {                                                          \
        std::cout << msg << std::endl;                                            \
        return nullptr;                                                           \
    }                                                                             \

#define CHECK_RET_RETURN_NULL(msg, ret)          \
    if(ret != ACL_ERROR_NONE) {                  \
        std::cout << msg << std::endl;           \
        return nullptr;                          \
    }                                            \
...
/**
 * @brief destroy dataset resource
 * @param [in] dataset: dataset
 * @param [in] flag: flag of if use dvpp
 * @return result
 **/
...
/**
 * @brief read file
 * @param [in] fileLocation: file location
 * @param [out] fileSize: size of the file
 * @return ptr of file memory
 **/
...
struct Config {
    ...
    char* modelData_ptr; // model work memory buffer in host
    void* devMem_ptr; // model work memory buffer in device
    void* weightMem_ptr; // model weight memory buffer
    std::string imgType;
