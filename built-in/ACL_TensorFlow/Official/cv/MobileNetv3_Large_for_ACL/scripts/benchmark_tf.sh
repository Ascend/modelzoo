...
preprocess()
{
	...
	cd $cur_dir
	python3 mobilenet_data_prepare.py --image_path=image --out_path=$dataset/imagenet_50000
...
collect_result()
{
	...
	python3 $cur_dir/bintofloat.py $cur_dir/../results/${modelType}/ $outputType > file.log
	
	echo "start parse mobilenetv3large" >> $testcase_dir/run.log
	
	python3 $cur_dir/result_statistical_mobilenet_tf.py $cur_dir/../results/${modelType}/ $trueValuePath >> $testcase_dir/run.log 2>&1
...
echo ""
	...
	--modelType           model type(mobilenetv3_large)
    ...
    --modelPath           (../../model/mobilenetv3_large_aipp.om)
    --dataPath            (../../datasets/imagenet_50000/)
    --trueValuePath       the path of true Value, default is ../../datasets/input_5w.csv
    -h/--help             Show help message
    Example:
    ./benchmark_tf.sh --batchSize=1  --outputType=fp32 --modelPath=../../model/mobilenetv3_large_aipp.om --dataPath=../../datasets/imagenet_50000/ --modelType=mobilenetv3_large --imgType=rgb --trueValuePath=../../datasets/input_5w.csv"
#    '
    exit 1
fi
...
trueValuePath=../../datasets/input_5w.csv
