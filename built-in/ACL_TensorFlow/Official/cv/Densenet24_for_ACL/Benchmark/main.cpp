...
std::vector<ReadImageCfg> Imgcfgvec;
std::vector<std::string> dyShape;
...
void getCommandLineParam(int argc, char** argv, Config& config)
{
    while (1) {
		...
		SplitString(config.dataDir, inputs, ',');
            ...
            case 'j':
                ...
                break;
	    case 'k':
		config.imgconfig = std::string(optarg);
		printf("[INFO]imgconfig = %s\n", config.imgconfig.c_str());
		break;
	    case 'l':
		config.useDynamic = atoi(optarg);
		printf("[INFO]useDynamic = %d\n", config.useDynamic);
		break;
	    case 'm':
		config.dynamicShape = std::string(optarg);
		SplitString(config.dynamicShape, dyShape, ';');
		printf("[INFO]dynamicShape = %s\n", config.dynamicShape.c_str());
		break;
            default:
                break;
        }
...
aclError Process()
{
    std::vector<std::vector<std::string>> dimsize(cfg.inputNum);
    if (dyShape.size() != 0){
        for (int i = 0; i < cfg.inputNum; i++){
	    SplitString(dyShape[i], dimsize[i], ',');
	    LOG("input shape: %s\n", dyShape[i].c_str());
	}
    }
    std::vector<std::vector<std::string>> fileNames(cfg.inputNum);
    for (int i = 0; i < inputs.size(); i++){
        ret = GetFiles(inputs[i], fileNames[i]);
        CHECK_RET(ret);
	LOG("file_names : %s\n", fileNames[i][0].c_str());
    }
    size_t fileNum = fileNames[0].size();
    LOG("fileNum:%zd\n",fileNames[0].size());
    struct timeval startTmp, endTmp;
    printf("======shape length %d, imgcfg path %s.\n", dyShape.size(),cfg.imgconfig.c_str());
    if (dyShape.size() == 0 && cfg.imgconfig != ""){
	ret = GetOriImgSize(cfg.imgconfig, Imgcfgvec);
	CHECK_RET(ret);
	LOG("Read ori image size success, Imgsize length:%d\n",Imgcfgvec.size());
    }
    ...
    while(loopCnt < cfg.loopNum) {
        ...
        for(size_t i = 0; i < fileNum / cfg.batchSize; i++) {
            ...
            std::vector<std::vector<std::string>> batchFileNames(cfg.inputNum);
	    LOG("init input batch %d start\n", processedCnt);
            for (size_t k = 0; k < cfg.inputNum && k < MODEL_INPUT_OUTPUT_NUM_MAX; k++) {
		for (int j = 0; j < cfg.batchSize; j++) {
                    batchFileNames[k].push_back(fileNames[k][i*cfg.batchSize+j]);
                }

	        if(cfg.useDvpp) {
		    ret = DvppInitInput(batchFileNames[k]);
		}
		else {
		    ret = InitInput(k, batchFileNames[k], dimsize[k]);
		}
            }
	...
	if (fileNum % cfg.batchSize != 0){
		...
		for(size_t i = (fileNum - fileNum % cfg.batchSize); i < fileNum; i++) {
		    batchFileNames[k].push_back(fileNames[k][i]);
		}
