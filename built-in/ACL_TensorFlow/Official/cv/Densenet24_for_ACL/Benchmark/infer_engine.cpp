...
extern std::vector<ReadImageCfg> Imgcfgvec;
extern std::vector<std::string> dyShape;
...
aclError InitInput(size_t index, std::vector<std::string> files, std::vector<std::string> dims)
{
    ...
    if (cfg.inputInfo[index].size == 0) {
        if (dyShape.size() == 0){
	    for (int i = 0; i < Imgcfgvec.size(); i++){
		Imgname = Imgcfgvec[i].ImgName;
		if (Imgname == files[0]){
	            img_width = Imgcfgvec[i].width;
		    img_height = Imgcfgvec[i].height;
		    Input_size = Imgcfgvec[i].InputSize;
		    break;
		}
            }
	    modelInputSize = Input_size;
	    LOG("input_size: %d \n",modelInputSize);
	    if (cfg.inputInfo[index].Format == 1){
		int64_t shape[] = {cfg.batchSize,img_height,img_width,3};
		inputDesc = aclCreateTensorDesc(ACL_FLOAT, 4, shape, ACL_FORMAT_NHWC);
	    }
	    if (cfg.inputInfo[index].Format == 0){
		    int64_t shape[]={cfg.batchSize,3,img_height,img_width};
		    inputDesc=aclCreateTensorDesc(ACL_FLOAT, 4, shape, ACL_FORMAT_NHWC);
	    }
	    if (inputDesc == nullptr){
		LOG("create inputdesc failed!\n");
	    }
	}else{
	    int64_t *shape = new int64_t[dims.size()];
	    size_t product = 1;
	    for (int i = 0; i < dims.size(); i++){
		product = product * atoi(dims[i].c_str());
		shape[i] = atoi(dims[i].c_str());
	    }
	    size_t Input_type = cfg.inputInfo[index].Type;
	    if (Input_type == 4){
		modelInputSize = product;
	    }
	    if(Input_type == 0){
		    modelInputSize=product * 4;
	    }

	    LOG("input_size: %d", modelInputSize);
	    inputDesc = aclCreateTensorDesc(ACL_FLOAT, dims.size(), shape, ACL_FORMAT_NHWC);
	    if (inputDesc == nullptr){
		LOG("create inputdesc failed!\n");
	    }
	}
    }else{
	modelInputSize = cfg.inputInfo[0].size;
	if (dyShape.size() == 0 && cfg.imgconfig != ""){
            for (int i = 0; i < Imgcfgvec.size(); i++){
		Imgname = Imgcfgvec[i].ImgName;
		if (Imgname == files[0]){
		    img_width = Imgcfgvec[i].width;
	...
	if (cfg.imgconfig == "" && dyShape.size() == 0){
	    int64_t *shape = new int64_t[cfg.inputInfo[index].dimCount];
	    for (size_t dimIdx = 0; dimIdx < cfg.inputInfo[index].dimCount; dimIdx++){
		shape[dimIdx] = cfg.inputInfo[index].dims[dimIdx];
	    }
            
	    inputDesc = aclCreateTensorDesc(ACL_FLOAT, cfg.inputInfo[index].dimCount, shape, ACL_FORMAT_NHWC);
	    if (inputDesc == nullptr){
	    LOG("create inputdesc failed!\n");
	    }
	}
    ...
    for (int i = 0; i < files.size(); i++) {
        ...
        std::string fileLocation = inputs[index] + "/" + files[i];
    ...
    ret = aclmdlAddDatasetBuffer(input_dataset, inputData);
    if (ret != ACL_ERROR_NONE) {
    ...
    inputDataframe.dataset = input_dataset;

    if (cfg.useDynamic){
	ret = aclmdlSetDatasetTensorDesc(inputDataframe.dataset, inputDesc, index);
	if (ret != ACL_ERROR_NONE){
	    LOG("set tensordesc failed. error=%d .\n",ret);
	    return ret;
	}
    }
