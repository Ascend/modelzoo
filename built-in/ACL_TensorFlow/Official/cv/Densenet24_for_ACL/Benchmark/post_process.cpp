...
aclError SaveBinPostprocess()
{
    ...
    for (size_t i = 0; i < cfg.outputNum; ++i) {
        ...
        uint32_t len;
	if (cfg.useDynamic) {
	    outputDesc[i] = aclmdlGetDatasetTensorDesc(output, i);
	    if (outputDesc[i] == nullptr) {
	        LOG("get output tensordesc failed.\n");
		return 1;
	    }

	    len = aclGetTensorDescSize(outputDesc[i]);
	    if (len == 0){
	        LOG("get output tensorsize failed.\n");
		return 1;
	    }
	}else{
            len = cfg.outputInfo[i].size;
        }
        LOG("tensor size is %d",len);
