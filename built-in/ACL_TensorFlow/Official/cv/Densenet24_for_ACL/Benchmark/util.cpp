...
aclError GetOriImgSize(std::string cfgfile, std::vector<ReadImageCfg>& Imagevec){

    std::ifstream sFile(cfgfile.c_str(), std::ifstream::in);
    if( !sFile.is_open()){
	...
	std::stringstream ssIn(lineText);
	ssIn >> Imgcfg.Id;
	ssIn >> Imgcfg.ImgName;
	ssIn >> Imgcfg.width;
	ssIn >> Imgcfg.height;
	ssIn >> Imgcfg.InputSize;

	Imagevec.push_back(Imgcfg);
    }
    sFile.close();
