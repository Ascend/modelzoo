...
infer_test()
{
    ...
    echo "./benchmark  --om $modelPath --batchSize $batchSize --modelType $modelType --imgType $imgType --deviceId $deviceId --loopNum $loopNum --useDvpp $useDvpp --imgInfo $imgInfoFile"
    ./benchmark --om $modelPath --batchSize $batchSize --modelType $modelType --imgType $imgType --deviceId $deviceId --loopNum $loopNum --useDvpp $useDvpp --imgInfo $imgInfoFile
    
    elif [[ $imgType != yuv && $imgType != rgb ]];then
        echo "para imgType $imgType invalid, must be yuv or rgb with useDvpp==$useDvpp"
        exit 1
    else
        echo "./benchmark --om $modelPath --batchSize $batchSize --modelType $modelType --imgType $imgType --deviceId $deviceId --loopNum $loopNum --framework $framework --useDvpp $useDvpp --imgInfo $imgInfoFile"
        ./benchmark --om $modelPath --batchSize $batchSize --modelType $modelType --imgType $imgType --deviceId $deviceId --loopNum $loopNum --framework $framework --useDvpp $useDvpp --imgInfo $imgInfoFile
    fi
}
...
collect_result()
{
    ...
    cat $testcase_dir/precision_results.log
}
...
do
    ...
    elif [[ $para == --imgInfoFile* ]];then
        imgInfoFile=`echo ${para#*=}`
    elif [[ $para == --classNamePath* ]];then
        classNamePath=`echo ${para#*=}`
    fi
done    
