...
def draw_box(boxes, img_path):
    ...
    fig.savefig(img_path + "-label.jpg")


def get_label_map(pipline_cfg):
    with open(pipline_cfg, "r") as fp:
        cfg = json.loads(fp.read())
    try:
        label_path = cfg["im_maskrcnn"]["mxpi_modelinfer0"]["props"][
            "labelPath"]
    except KeyError:
        logging.error("The pipeline config(%s) is invalid.", pipline_cfg)
        return False

    with open(label_path, "r") as fp:
        data = fp.readlines()

    data = [d.strip() for d in data if d and not d.startswith("#")]
    return {i: d for i, d in enumerate(data)}


def drew_label(img_path, label_map):
    with open(RESULT_TEMP_FILE, "r") as fp:
    ...
    for bbox in data:
        bound_box = dict(
            label=label_map.get(int(bbox["classId"]), "unknown"),
            confidence=float(bbox["confidence"]),
            x0=int(float(bbox["x0"])),
            y0=int(float(bbox["y0"])),
        ...
        mask = bbox["mask"]
        if len(mask) != bound_box["width"] * bound_box["height"]:
            print("The result data is error.")
            return
        mask = [255 if i == "1" else 0 for i in mask]
        mask = np.array(mask)
        mask = mask.reshape(bound_box["height"], bound_box["width"])
        bound_box["mask"] = mask
        bound_boxes.append(bound_box)
    draw_box(bound_boxes, img_path)
