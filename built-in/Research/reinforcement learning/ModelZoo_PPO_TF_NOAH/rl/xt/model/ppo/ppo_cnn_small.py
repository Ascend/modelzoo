...
from xt.model.tf_compat import K, Conv2D, Dense, \
    Flatten, Input, Lambda, Model, Adam, Activation
...
class PPOCnnSmall(XTModel):
    ...
    def __init__(self, model_info):
        ...
        super(PPOCnnSmall, self).__init__(model_info)
    ...
    def create_model(self, model_info):
        ...
        losses = {
            "output_actions_raw": ppo_loss_new(advantage=advantage, old_prediction=old_prediction),
...
def ppo_loss_new(advantage, old_prediction):
    ...
    def loss(y_true, y_pred):
        ...
        return -K.mean(
            K.minimum(ratio * advantage,
                      K.clip(ratio, min_value=1 - LOSS_CLIPPING, max_value=1 + LOSS_CLIPPING) * advantage) -
            ENTROPY_LOSS * (y_pred * K.log((y_pred + 1e-10))), 1)
