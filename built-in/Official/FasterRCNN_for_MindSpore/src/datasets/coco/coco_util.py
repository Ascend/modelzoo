...
import mmcv
from ...util import summary_init
from src.config import config

def coco_eval(outputs, ann_file, result_types, max_dets=(100, 300, 1000), single_result=False):
    """coco eval for fasterrcnn"""
    coco = COCO(ann_file)
    result_files = results2json(coco, outputs, "./results.pkl")
...
def create_coco_label(is_training):
    ...
    anno_json = os.path.join(dataset_root, config.instance_set.format(data_type))
    ...
    for img_id in image_ids:
        ...
        image_path = os.path.join(dataset_root, data_type, file_name)
        annos = []
        for label in anno:
            bbox = label["bbox"]
