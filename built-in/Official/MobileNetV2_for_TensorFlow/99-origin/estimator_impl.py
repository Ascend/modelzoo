...
class EstimatorImpl:
	...
	def main(self):
		...
		hooks = [
			...
			# tf.train.LoggingTensorHook(['train_accuracy', 'train_loss'], every_n_iter=self.env.FLAGS.log_every_n_steps)
		]
		if self.env.FLAGS.enable_hvd:
			hooks.append(self.env.hvd.BroadcastGlobalVariablesHook(0))
		# assert len(hooks) == 2

		estimator_config = tf.ConfigProto(allow_soft_placement=True)
