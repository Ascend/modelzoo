...
def model_fn(features, labels, mode, params):
  ...
  if is_training:
    ...
    # for GPU
    scaled_lr = FLAGS.base_learning_rate * (FLAGS.train_batch_size * hvd.size() / 256.0)
    # for GPU
...
def main(unused_argv):
  ...
  # for GPU
  estimator_config = tf.ConfigProto(allow_soft_placement=False)

  estimator_config.gpu_options.visible_device_list = str(hvd.local_rank())
  estimator_config.intra_op_parallelism_threads = 1
  estimator_config.inter_op_parallelism_threads = 5

  print("XLA is activated.")
  estimator_config.graph_options.optimizer_options.global_jit_level = tf.OptimizerOptions.ON_1
  ...
  config = tf.estimator.RunConfig(
    save_checkpoints_steps=save_checkpoints_steps if hvd.rank()==0 else None,
    log_step_count_steps=FLAGS.log_step_count_steps,
  ...
  # for GPU
  params = dict(
          steps_per_epoch=FLAGS.num_train_images / (FLAGS.train_batch_size * hvd.size()),
