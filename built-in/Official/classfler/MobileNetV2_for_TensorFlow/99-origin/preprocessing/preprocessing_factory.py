...
def get_preprocessing(name, is_training=False, use_grayscale=False, add_image_summaries=False):
...
def get_preprocessing_xling(is_training=False, use_grayscale=False, add_image_summaries=False):
	if is_training:
		def preprocessing_fn1(image, output_height, output_width, **kwargs):
			return inception_preprocessing.preprocess_for_train1(
				image, output_height, output_width, bbox=None,
				fast_mode=kwargs.get('fast_mode', True),
				add_image_summaries=add_image_summaries, random_crop=True,
				use_grayscale=use_grayscale)

		def preprocessing_fn2(distorted_image, height, width):
			return inception_preprocessing.preprocess_for_train2(
				distorted_image, height, width)

		return [preprocessing_fn1, preprocessing_fn2]
	else:
		def preprocessing_fn(image, output_height, output_width, **kwargs):
