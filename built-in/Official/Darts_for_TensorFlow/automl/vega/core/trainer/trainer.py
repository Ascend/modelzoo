...
class Trainer(DistributedWorker):
    ...
    def train_process(self):
        ...
        """
        print('trainer {} begin process'.format(self.worker_id))
    ...
    def _init_distributed_setting(self):
        print('rank_size: {}'.format(os.environ.get('RANK_SIZE')),
                  'device_id: {}'.format(os.environ.get('DEVICE_ID')),
                  'rank_id: {}'.format(os.environ.get('RANK_ID')),
                  'rank_table_file: {}'.format(os.environ.get('RANK_TABLE_FILE')))
        if not self.distributed:
            return
        if vega.is_npu_device():
            import json
            with open(os.environ.get('RANK_TABLE_FILE'), 'r') as f:
                rank_table_json = json.loads(f.read())
            print(rank_table_json)
    ...
    def _init_dataloader(self, mode, loader=None):
        ...
        if vega.is_torch_backend():
            ...
            if mode == 'train' and self.distributed:
    ...
    def _train_loop(self):
        ...
        if self.skip_train:
            return
        if not self.config.train_in_once:
            ...
            logging.info('trainer {} metrics: {}'.format(self.worker_id, self.valid_metrics.results))
        if self.distributed:
    ...
    def _init_npu_estimator(self, sess_config):
        ...
        # profiling_options = ['task_trace', 'training_trace']
        # profiling_config = ProfilingConfig(enable_profiling=True, enable_options=profiling_options)
