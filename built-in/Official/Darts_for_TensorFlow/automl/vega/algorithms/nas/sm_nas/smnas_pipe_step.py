...
class SMNasPipeStep(PipeStep):
    ...
    def do(self):
        ...
        logger.info("SMNasPipeStep started")
        while not self.generator.is_completed:
            id, smnas_sample = self.generator.search_alg.search()
            cls_trainer = ClassFactory.get_cls('trainer')
            trainer = cls_trainer(smnas_sample, id=id)
