...
context.set_context(mode=context.GRAPH_MODE, enable_auto_mixed_precision=True,
                    device_target="Davinci", save_graphs=True, device_id=dev_id)
...
def parse_args(cloud_args={}):
    ...
    parser.add_argument('--per_batch_size', default=1, type=int, help='batch size per gpu')
    # network related
    ...
    # logging related
    parser.add_argument('--log_path', type=str, default='../output', help='path to save log')
...
def test(cloud_args={}):
    ...
    if args.rank == 0:
        if not os.path.exists(args.cache_dir):
            ...
            args.logger.info('create cache dir {}'.format(args.cache_dir))
    ...
    for model in args.models:
        ...
        network = get_network(args.backbone, args.num_classes, is_train=False)
        ...
        if args.is_distributed:
            ...
            args.logger.info('rank {} ready'.format(args.rank))
            while True:
                rank_ok = True
                for other_rank in range(args.group_size):
                    ...
                    if not os.path.exists(top1_correct_npy) or not os.path.exists(
                            top5_correct_npy) or not os.path.exists(img_tot_npy):
                        rank_ok = False
                if rank_ok:
                    break
                else:
        ...
        args.logger.info('results={}'.format(results))
