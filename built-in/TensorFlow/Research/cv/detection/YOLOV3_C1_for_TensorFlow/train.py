...
class YoloTrain(object):
    ...
    def __init__(self):
        ...
        config = tf.ConfigProto(allow_soft_placement=True)  # 如果没有tf.ConfigProto，需要手工添加该行
        custom_op = config.graph_options.rewrite_options.custom_optimizers.add()
        custom_op.name = "NpuOptimizer"
        custom_op.parameter_map["dynamic_input"].b = True
        #custom_op.parameter_map["dynamic_graph_execute_mode"].s = tf.compat.as_bytes("dynamic_execute")
        #custom_op.parameter_map["dynamic_inputs_shape_range"].s = tf.compat.as_bytes("getnext:[6 ,320~608, 320~608, 3],[6,1~200,1~200,3,25],[6,1~200,1~200,3,25],[6,1~200,1~200,3,25],[6,150,4],[6,150,4],[6,150,4],[]")
        ...
        with tf.name_scope('define_first_stage_train'):
            ...
            #first_stage_optimizer = npu_tf_optimizer(tf.train.AdamOptimizer(self.learn_rate).minimize(self.loss, var_list=self.first_stage_trainable_var_list))
            print(type(global_step_update))
        ...
        with tf.name_scope('define_second_stage_train'):
            ...
            #second_stage_optimizer = npu_tf_optimizer(tf.train.AdamOptimizer(self.learn_rate).minimize(self.loss, var_list=second_stage_trainable_var_list))
