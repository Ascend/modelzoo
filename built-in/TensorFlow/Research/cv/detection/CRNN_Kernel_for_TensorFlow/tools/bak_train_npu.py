...
def train_shadownet(dataset_dir, weights_path, char_dict_path, ord_map_dict_path,save_dir,args, need_decode=False):
    ...
    x, y = np.meshgrid(np.arange(CFG.ARCH.MAX_LENGTH), np.arange(CFG.TRAIN.BATCH_SIZE))
    indexes = np.concatenate([y.flatten()[:, None], x.flatten()[:, None]], axis=1)
    indexes = tf.constant(indexes, dtype=tf.int64)
    train_labels = tf.SparseTensor(indexes, tf.reshape(train_labels, [-1]), np.array([CFG.TRAIN.BATCH_SIZE, CFG.ARCH.MAX_LENGTH], dtype=np.int64))

    val_images, val_labels, val_images_paths,val_labels_length = val_dataset.inputs(
        batch_size=CFG.TRAIN.BATCH_SIZE
    )
    val_labels = tf.SparseTensor(indexes, tf.reshape(val_labels, [-1]), np.array([CFG.TRAIN.BATCH_SIZE, CFG.ARCH.MAX_LENGTH], dtype=np.int64))
    ...
    #loss_scale_manager = FixedLossScaleManager(loss_scale=32768*4)

    #if rank_size == 1:
    #  opt = NPULossScaleOptimizer(optimizer, loss_scale_manager)
    #else:
    #  opt = NPULossScaleOptimizer(optimizer, loss_scale_manager, is_distributed=True)
    ...
    with tf.control_dependencies(update_ops):
        ...
        grads_and_vars = opt.compute_gradients(train_ctc_loss)
        ...
        while epoch < train_epochs:
            ...
            if need_decode and epoch % 500 == 0:
                ...
                if epoch % 10 == 0:
                    ...
                    print('Epoch_Train: {:d} cost= {:9f}'.format(epoch , train_ctc_loss_value))
