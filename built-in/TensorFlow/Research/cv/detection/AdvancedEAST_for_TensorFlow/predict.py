...
def predict(east_detect, img_path, pixel_threshold, quiet=False):
    ...
    with Image.open(img_path) as im:
        ...
        for (i, j) in zip(activation_pixels[0], activation_pixels[1]):
            ...
            draw.line([((px - (0.5 * cfg.pixel_size)), (py - (0.5 * cfg.pixel_size))), ((px + (0.5 * cfg.pixel_size)), (py - (0.5 * cfg.pixel_size))), ((px + (0.5 * cfg.pixel_size)), (py + (0.5 * cfg.pixel_size))), ((px - (0.5 * cfg.pixel_size)), (py + (0.5 * cfg.pixel_size))), ((px - (0.5 * cfg.pixel_size)), (py - (0.5 * cfg.pixel_size)))], width=line_width, fill=line_color)
        im.save((img_path + '_act.jpg'))
        quad_draw = ImageDraw.Draw(quad_im)
        txt_items = []
        for (score, geo, s) in zip(quad_scores, quad_after_nms, range(len(quad_scores))):
            if (np.amin(score) > 0):
                quad_draw.line([tuple(geo[0]), tuple(geo[1]), tuple(geo[2]), tuple(geo[3]), tuple(geo[0])], width=2, fill='red')
...
def parse_args():
    ...
    parser.add_argument('--path', '-p', default='demo/012.png', help='image path')
