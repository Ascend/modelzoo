...
class East():
    ...
    def __init__(self):
        self.input_img = Input(name='input_img', shape=(None, None, cfg.num_channels), dtype='float32')
    ...
    def g(self, i):
        assert ((i + self.diff) in cfg.feature_layers_range), (('i=%d+diff=%d not in ' % (i, self.diff)) + str(cfg.feature_layers_range))
