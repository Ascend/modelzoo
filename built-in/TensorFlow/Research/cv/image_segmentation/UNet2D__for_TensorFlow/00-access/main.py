...
def main(_):
    ...
    dataset = Dataset(data_dir=params.data_dir,
                      ...
                      # for NPU
                      #gpu_id=hvd.rank(),
                      gpu_id=int(os.getenv("RANK_ID")),
                      #num_gpus=hvd.size(),
                      num_gpus=int(os.getenv("RANK_SIZE")),
                      # for NPU
                      seed=params.seed)
    ...
    if 'train' in params.exec_mode:
        ...
        hooks = [TrainingHook(logger,
        # for NPU
                              max_steps=max_steps,
                              log_every=params.log_every)]
        # for NPU
        #if params.benchmark and hvd.rank() == 0:
        if params.benchmark and int(os.getenv("RANK_ID")) == 0:
        # for NPU
    ...
    if 'evaluate' in params.exec_mode:
        ...
        # for NPU
            results = estimator.evaluate(input_fn=lambda: dataset.eval_fn(count=params.batch_size), steps=dataset.eval_size)
            logger.log(step=(),
