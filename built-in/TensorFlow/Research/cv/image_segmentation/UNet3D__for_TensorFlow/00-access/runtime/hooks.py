    ...
    if 'train' in params.exec_mode:
        ...
        if params.benchmark:
            hooks += [ProfilingHook(warmup_steps=params.warmup_steps,
                                    ...
                                    global_batch_size=int(os.getenv("RANK_SIZE")) * params.batch_size,
                                    # for NPU
                                    logger=logger,
                                    mode='train')]
        else:
            hooks += [TrainingHook(log_every=params.log_every,
                                   logger=logger,
                                   tensor_names=['total_loss_ref:0', "global_step:0"])]
        return hooks
...
class TrainingHook(tf.estimator.SessionRunHook):
    ...
    def after_run(self,
        ...
        if self._step % self._log_every == 0:
            ...
            tf.logging.warning("steps:%s, total_loss:%s, global_step:%s", str(self._step), str(run_values.results[0]), str(run_values.results[1]))
        self._step += 1
...
def process_performance_stats(timestamps, batch_size, mode):
    ...
    timestamps_ms = 1000 * timestamps / 10
    # for NPU
