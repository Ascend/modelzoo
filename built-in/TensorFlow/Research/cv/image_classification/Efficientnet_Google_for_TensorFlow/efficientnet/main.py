...
flags.DEFINE_bool(
    'use_tpu', default=False, # npu modify
...
flags.DEFINE_integer(
    'num_train_images', default=160000, help='Size of training data set.')

flags.DEFINE_integer(
    'num_eval_images', default=1000, help='Size of evaluation data set.')
...
def model_fn(features, labels, mode, params):
  ...
  if is_training:
    ...
    with tf.control_dependencies(update_ops):
      ...
      train_op = distributedOptimizer.minimize(loss=loss, global_step=global_step)  # 最小化loss
      ############## npu modify end ###############
  ...
  if mode == tf.estimator.ModeKeys.EVAL:
    def metric_fn(labels, logits):
      ...
      in_top_5 = tf.cast(tf.nn.in_top_k(tf.cast(logits, tf.float32), labels, 5), tf.float32)
...
def main(unused_argv):
  ...
  if FLAGS.use_async_checkpointing:
    ...
    save_checkpoints_steps = 10000 #max(100, FLAGS.iterations_per_loop)
  ...
  if FLAGS.mode == 'eval':
    ...
    #     tmp_step = int(os.path.basename(ckpt).split('-')[1])
    #     if tmp_step > current_step:
    #         current_step = tmp_step
