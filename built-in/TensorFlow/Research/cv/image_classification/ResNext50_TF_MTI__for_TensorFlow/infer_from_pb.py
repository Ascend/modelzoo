...
def parse_args():
    ...
    parser.add_argument('--batchsize', default=1,
                        help="""batchsize""")
    parser.add_argument('--model_path', default='pb/resnext50_tf_910.pb',
                        help="""pb path""")
...
def crop_and_resize_image(image, height, width):
    with tf.name_scope('crop_and_resize'):
        ...
        image = tf.image.crop_and_resize(image[None, :, :, :], bbox[None, :], [0], [height, width])[0]
        return image
...
def image_process(image_path, label_file):
    ...
    for file in os.listdir(image_path):
        with tf.Session().as_default():
            ...
            image = crop_and_resize_image(image, 224, 224)
