...
aclError DvppInitInput(std::vector<std::string> files)
{
    ...
    for (int i = 0; i < files.size(); i++) {
        ...
        //ret = aclrtMalloc(&jpeg_dev_mem_out_ptr, outputBuffSize, ACL_MEM_MALLOC_HUGE_ONLY);
        ...
        //acldvppRoiConfig* decode_area = nullptr;
        ...
        gettimeofday(&func_start, NULL);
        //ret = acldvppJpegDecodeAsync(dvpp_channel_desc, jpeg_input_desc, jpeg_output_desc, decode_area,stream);
        ...
        acldvppPicDesc *cropInputDesc_ = nullptr; // resize output desc
        ...
        centerCropConfig = InitCropCenterRoiConfig(newInputWidth, newInputHeight, 224, 224);
        void *vpcOutBufferDev = nullptr;
        uint32_t vpcOutBufferSize = 224 * 224 * 3 / 2;
        uint32_t vpcOutputWidthStride = (224 + (16 - 1)) / 16 * 16;
        uint32_t vpcOutputHeightStride = (224 + (2 - 1)) / 2 * 2;

        vpcOutBufferDev = (void*)ptr;
        centerCropOutputDesc = createDvppPicDesc(vpcOutBufferDev, PIXEL_FORMAT_YUV_SEMIPLANAR_420, 224, 224, vpcOutputWidthStride, vpcOutputHeightStride, vpcOutBufferSize);
        ...
        if (cropOutputDesc != nullptr) {
            ...
            cropOutputDesc = nullptr;
        }
        if (centerCropOutputDesc != nullptr) {
            acldvppDestroyPicDesc(centerCropOutputDesc);
            centerCropOutputDesc = nullptr;
        }
