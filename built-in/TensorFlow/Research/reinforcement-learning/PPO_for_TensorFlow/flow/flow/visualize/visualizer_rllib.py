        ...
        for _ in range(env_params.horizon):
            ...
            if multiagent:
                ...
                for agent_id in state.keys():
                    if use_lstm:
                        if agent_id not in state_init:
                            state_init[agent_id] = [np.zeros(size, np.float32),
                                       np.zeros(size, np.float32)]
...
def create_parser():
    ...
    parser.add_argument(
        'result_dir', 
        type=str, 
        # default='~/ray_results/baseline-1x1-442-lstm/PPO_MultiTrafficLightGridPOEnv-v0_0_2020-03-31_09-40-26qmw12700/', 
    ...
    parser.add_argument(
        ...
        default='no_render',
        # default='sumo_web3d',
        help='Pick the render mode. Options include no_render, sumo_web3d, '
             'rgbd and sumo_gui')
    parser.add_argument(
        '--save_render',
        action='store_true',
