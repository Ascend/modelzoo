...
class QMixAlg(Algorithm):
    ...
    def __init__(
        ...
        self.train_time_track = deque(maxlen=20) 
    ...
    def restore(self, model_name, model_weights=None):
        ...
        check_keep_model(os.path.dirname(model_name), keep_num=50)
    ...
    def train(self, **kwargs):
        ...
        #    np.array(batch_trajectories).astype(np.float32),
        ...
        loss_val = self.actor.train(
            np.array(batch_trajectories).astype(np.float32),
            np.array(list([max_ep_t for _ in range(batch.batch_size * self.n_agents)])).astype(np.float32),
            np.array(avail_actions).astype(np.float32),
            np.array(actions).astype(np.float32),
            np.array(batch["state"][:, :-1]).astype(np.float32),
            np.array(batch["state"][:, 1:]).astype(np.float32),
            np.array(rewards).astype(np.float32),
            np.array(terminated).astype(np.float32),
            np.array(mask).astype(np.float32),
        )
        #self.train_time_track.append(time()-_t)
        #print(self.train_time_track, np.nanmean(self.train_time_track))
...
def check_keep_model(model_path, keep_num=100):
    """keep the remote model count"""
    target_file = glob.glob(os.path.join(model_path, "actor*".format(model_path)))
    if len(target_file) > keep_num:
        sorted_model = sorted(target_file, reverse=True)
        if "index" in "-".join(sorted_model[:3]):
            keep_num *= 2
