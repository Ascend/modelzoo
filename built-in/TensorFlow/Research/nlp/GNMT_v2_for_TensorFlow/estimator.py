...
class ModelFnFactory(object):
  ...
  def _shard_inputs(self, features, num_towers):
    ...
    for i in range(num_towers):
      print("==================sources========", sources[i])
      print("==================target_input========", target_input[i])
      print("==================target_output========", target_output[i])
      print("==================source_sequence_length========", source_sequence_lengths[i])
      print("==================target_sequence_length========", target_sequence_lengths[i])
...
def get_sacrebleu(trans_file, detokenizer_file):
  ...
  # run sacrebleu
  cmd = ("cat %s | /usr/local/python3.7.5/bin/sacrebleu -t wmt14/full -l en-de --score-only -lc --tokenize"
...
def train_fn(hparams):
  ...
  #dump_config = DumpConfig(enable_dump=True, dump_path="/opt/npu1/GNMT-dump", dump_step="0-3", dump_mode="all")
  #dump_config = DumpConfig(enable_dump_debug = True, dump_path = "/home/w00348617/GNMT_weilixia/dump", dump_debug_mode = "all" )
  config = NPURunConfig(
  ...
  import hook
  train_hooks.append(hook._LogSessionRunHook())
  estimator.train(
      input_fn=input_fn,
      max_steps=max_steps,
      hooks=train_hooks,
