...
# XXX
import npu_contrib_seq2seq_decoder
import npu_contrib_seq2seq_basic_decoder
import npu_contrib_seq2seq_beam_search_decoder

# XXX
from static_utils.rnn import static_rnn
from static_utils.stack_utils import npu_stack, npu_unstack
from static_utils.static_decoder_utils import static_decoder
from static_utils.basic_decoder import BasicDecoder
from static_utils.helper import TrainingHelper
...
class BaseModel(object):
  ...
  def _set_train_or_infer(self, res, reverse_target_vocab_table, hparams):
    ...
    if self.mode == tf.contrib.learn.ModeKeys.TRAIN:
      ...
      self.word_count = tf.reduce_sum(
          # XXX
          self.iterator.real_src_seq_len) + tf.reduce_sum(
              self.iterator.real_tgt_seq_len)
    ...
    if self.mode == tf.contrib.learn.ModeKeys.TRAIN:
      ...
      if os.environ.get('USE_NPU') in [ 'True', '1' ]:
        ...
        if int(rank_size) > 1:
          gradients = [ hccl_ops.allreduce(grad, "sum") for grad in gradients ]
      ...
      # XXX
      grads_and_vars = list(zip(clipped_grads, params))
      self.update = opt.apply_gradients(grads_and_vars, global_step=self.global_step)
  ...
  # XXX
  def _get_train_maximum_iterations(self, hparams):
    """Maximum decoding steps at training time."""
    maximum_iterations = hparams.tgt_max_len + 1
    utils.print_out("  decoding maximum_iterations %d (for train)" % maximum_iterations)
    return maximum_iterations
    ...
    with tf.variable_scope("decoder") as decoder_scope:
      ...
      if self.mode != tf.contrib.learn.ModeKeys.INFER:
        ...
        helper = TrainingHelper(
            decoder_emb_inp, iterator.real_tgt_seq_len,
        ...
        # Dynamic decoding
        # XXX
        maximum_iterations = self._get_train_maximum_iterations(hparams)
        outputs, final_context_state = static_decoder(
            my_decoder,
            # XXX
      ...
      ## Inference
        ...
        if infer_mode == "beam_search":
          ...
          # XXX
          my_decoder = npu_contrib_seq2seq_beam_search_decoder.BeamSearchDecoder(
              cell=cell,
        ...
        if infer_mode != "beam_search":
          # XXX
          my_decoder = npu_contrib_seq2seq_basic_decoder.BasicDecoder(
              cell,
              helper,
              decoder_initial_state,
        ...
        # Dynamic decoding
        # XXX
        outputs, final_context_state, _ = npu_contrib_seq2seq_decoder.dynamic_decode(
            my_decoder,
            maximum_iterations=maximum_iterations,
            output_time_major=self.time_major,
            swap_memory=True,
  ...
  def _softmax_cross_entropy_loss(
    ...
    if self.num_sampled_softmax > 0:
      # XXX
      print('Error: Not analyzed parameter: --num_sampled_softmax')
      exit()
  ...
  def _compute_loss(self, logits, decoder_cell_outputs):
    ...
    target_weights = tf.sequence_mask(
        # XXX
        self.iterator.real_tgt_seq_len, max_time, dtype=self.dtype)
    if self.time_major:
