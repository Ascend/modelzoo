...
# XXX
from static_utils.rnn_cell_impl import BasicLSTMCell
from static_utils.rnn_cell_impl import MultiRNNCell
from static_utils.rnn_cell_impl import DropoutWrapper
from static_utils.rnn_cell_impl import ResidualWrapper
...
def create_or_load_model(model, model_dir, session, name):
  ...
  if os.environ.get('USE_NPU') in [ 'True', '1' ]:
    ...
    if int(rank_size) > 1:
      ...
      broadcasts = hccl_ops.broadcast(inputs, 0)
      for i, _ in enumerate(inputs):
         session.run(tf.assign(inputs[i],broadcasts[i]))
