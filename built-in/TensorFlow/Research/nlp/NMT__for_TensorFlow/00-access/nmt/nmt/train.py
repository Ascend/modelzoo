...
def update_stats(stats, start_time, step_result):
  ...
  #add by c00314821 for print loss
  print('\n-------here is loss:',output_tuple.train_loss)
  # Update statistics
...
def print_step_info(prefix, global_step, info, result_summary, log_f):
  ...
  utils.print_out(
      "%sstep: %d lr: %g step-time: %.2fms wps: %.2fK ppl: %.2f gN: %.2f device: %s, %s" %
      (prefix, global_step, info["learning_rate"], info["avg_step_time"]*1000,
       info["speed"], info["train_ppl"], info["avg_grad_norm"], device_id,
       time.ctime()),
      log_f)
...
def train(hparams, scope=None, target_session=""):
  ...
  if os.environ.get('USE_NPU') in [ 'True', '1' ]:
      ...
      #add by jwx827641 20200402 版本接口变化
      #custom_op.parameter_map["enable_auto_mix_precision"].b=True
      custom_op.parameter_map["precision_mode"].s = tf.compat.as_bytes("allow_mix_precision")
      #add by jwx827641 20200402
      #custom_op.parameter_map["enable_auto_mix_precision"].b=True
      #custom_op.parameter_map["stream_max_parallel_num"].s = tf.compat.as_bytes("DNN_VM_TF:20")
      custom_op.parameter_map["stream_max_parallel_num"].s = tf.compat.as_bytes("DNN_VM_AICPU:20")
  ...
  while global_step < num_train_steps:
    ...
    if global_step - last_stats_step >= steps_per_stats:
      ...
      if global_step == num_train_steps:
        print_step_info("# Best bleu, ", global_step, info, get_best_results(hparams),
                      log_f)
      if is_overflow:
	    # XXX
        print_step_info("# Best bleu, ", global_step, info, get_best_results(hparams),
