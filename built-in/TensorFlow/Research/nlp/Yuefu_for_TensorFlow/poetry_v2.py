...
def top_k_logits(logits, k, history_mask, last_logit):
    ...
    history_mask = tf.cond(tf.math.logical_or(tf.equal(last_logit, tensor0), tf.equal(last_logit, tensor1)),
                           lambda: history_mask,
                           lambda: history_mask * tf.one_hot([last_logit] * logits_shape[0], depth=logits_shape[1],
                                                             on_value=1e3, off_value=1.0, dtype=logits.dtype))
    ...
    def __init__(self,
        ...
        def get_predict_output(bert_config, embedding_table, sequence_output, history_mask, last_logit):
            ...
            with tf.variable_scope("cls/predictions", reuse=tf.AUTO_REUSE):
                ...
                # output_id = tf.argmax(topk_output_ids, axis=1)
                # output_id = tf.cast(output_id, dtype=tf.int32)
