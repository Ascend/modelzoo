...
def train(args, unknow_args=None):
    batch_size = 64
    steps = 1000
    epochs = 5
    ...
    if args.loss_scale_flag != 0 ：
        if args.loss_scale_value == 0:
            loss_scale_manager = ExponentialUpdateLossScaleManger(init_loss_scale=2**32, incr_every_n_steps=1000, decr_every_n_nan_or_inf=2, decr_ratio=0.5)
    ...
    with tf.Session(config=npu_session_config_init()) as sess:
        ...
        with open(os.path.join(output_path, "performance_precision.txt"), "w") as fw:
            write_str = "Final Performance secs/step : " + str(round(cost_time, 4))
            print(str(write_str))
            fw.write(write_str)
            fw.write('\r\n')
            ...
            write_str = "Final Training Duration(s) : " + str(round(e2e_cost_time, 4))
            print(str(write_str))
            fw.write(write_str)
            fw.write('\r\n')
