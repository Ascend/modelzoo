...
if (os.name == 'nt'):
    ...
    root_path = 'C:\\workspace\\python\\BERT_Base'
else:
    bert_path = '/home/macan/ml/data/chinese_L-12_H-768_A-12'
    root_path = '/home/macan/ml/workspace/BERT_Base2'
flags = tf.flags
FLAGS = flags.FLAGS
flags.DEFINE_string('data_dir', os.path.join(os.path.join(root_path, 'data'), 'classification'), 'The input data dir. Should contain the .tsv files (or other data files) for the task.')
flags.DEFINE_string('bert_config_file', os.path.join(bert_path, 'bert_config.json'), 'The config json file corresponding to the pre-trained BERT model. This specifies the model architecture.')
...
flags.DEFINE_string('output_dir', os.path.join(os.path.join(root_path, 'output'), 'classification'), 'The output directory where the model checkpoints will be written.')
...
flags.DEFINE_integer('max_seq_length', 202, 'The maximum total input sequence length after WordPiece tokenization. Sequences longer than this will be truncated, and sequences shorter than this will be padded.')
...
class ThuProcessor(DataProcessor):
    ...
    def _create_examples(self, lines, set_type):
        ...
        for (i, line) in enumerate(lines):
            ...
            examples.append(InputExample(guid=guid, text_a=text_a, label=label, text_b=None))
        return examples
...
def file_based_input_fn_builder(input_file, seq_length, num_label, is_training, drop_remainder):
    ...
    name_to_features = {'input_ids': tf.FixedLenFeature([seq_length], tf.int64), 'input_mask': tf.FixedLenFeature([seq_length], tf.int64), 'segment_ids': tf.FixedLenFeature([seq_length], tf.int64), 'label_ids': tf.FixedLenFeature([], tf.int64)}
...
def main(_):
    ...
    run_config = tf.estimator.RunConfig(model_dir=FLAGS.output_dir, save_summary_steps=FLAGS.save_summary_steps, save_checkpoints_steps=FLAGS.save_checkpoints_steps, keep_checkpoint_max=5, log_step_count_steps=500, session_config=npu_config_proto(config_proto=tf.ConfigProto(log_device_placement=True)))
    train_examples = None
    ...
    model_fn = model_fn_builder(bert_config=bert_config, num_labels=len(label_list), init_checkpoint=FLAGS.init_checkpoint, learning_rate=FLAGS.learning_rate, num_train_steps=num_train_steps, num_warmup_steps=num_warmup_steps)
    params = {'batch_size': FLAGS.train_batch_size}
    estimator = tf.estimator.Estimator(model_fn=model_fn, config=npu_run_config_init(run_config=run_config), params=params)
    if FLAGS.do_train:
        train_file = os.path.join(FLAGS.output_dir, 'train.tf_record')
        file_based_convert_examples_to_features(train_examples, label_list, FLAGS.max_seq_length, tokenizer, train_file, 'train')
        logger.info('***** Running training *****')
        logger.info('  Num examples = %d', len(train_examples))
        logger.info('  Batch size = %d', FLAGS.train_batch_size)
        logger.info('  Num steps = %d', num_train_steps)
        train_input_fn = file_based_input_fn_builder(input_file=train_file, seq_length=FLAGS.max_seq_length, num_label=len(label_list), is_training=True, drop_remainder=True)
        estimator.train(input_fn=train_input_fn, max_steps=num_train_steps, hooks=npu_hooks_append())
    if FLAGS.do_eval:
        eval_examples = processor.get_dev_examples(FLAGS.data_dir)
        eval_file = os.path.join(FLAGS.output_dir, 'eval.tf_record')
        file_based_convert_examples_to_features(eval_examples, label_list, FLAGS.max_seq_length, tokenizer, eval_file, 'eval')
        logger.info('***** Running evaluation *****')
        logger.info('  Num examples = %d', len(eval_examples))
        logger.info('  Batch size = %d', FLAGS.eval_batch_size)
        eval_input_fn = file_based_input_fn_builder(input_file=eval_file, seq_length=FLAGS.max_seq_length, num_label=len(label_list), is_training=False, drop_remainder=False)
    ...
    if FLAGS.do_predict:
        ...
        file_based_convert_examples_to_features(predict_examples, label_list, FLAGS.max_seq_length, tokenizer, predict_file, 'test')
        logger.info('***** Running prediction*****')
        logger.info('  Num examples = %d', len(predict_examples))
        logger.info('  Batch size = %d', FLAGS.predict_batch_size)
        predict_input_fn = file_based_input_fn_builder(input_file=predict_file, seq_length=FLAGS.max_seq_length, num_label=len(label_list), is_training=False, drop_remainder=False)
