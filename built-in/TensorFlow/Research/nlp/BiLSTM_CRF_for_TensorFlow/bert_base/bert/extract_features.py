...
def input_fn_builder(features, seq_length):
    ...
    def input_fn(params):
        ...
        d = tf.data.Dataset.from_tensor_slices({'unique_ids': tf.constant(all_unique_ids, shape=[num_examples], dtype=tf.int32), 'input_ids': tf.constant(all_input_ids, shape=[num_examples, seq_length], dtype=tf.int32), 'input_mask': tf.constant(all_input_mask, shape=[num_examples, seq_length], dtype=tf.int32), 'input_type_ids': tf.constant(all_input_type_ids, shape=[num_examples, seq_length], dtype=tf.int32)})
...
def main(_):
    ...
    estimator = tf.contrib.tpu.TPUEstimator(use_tpu=False, model_fn=model_fn, config=npu_run_config_init(run_config=run_config), predict_batch_size=FLAGS.batch_size, eval_on_tpu=False, export_to_tpu=False)
...
def convert_lst_to_features(lst_str, seq_length, tokenizer, logger, is_tokenized=False, mask_cls_sep=False):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        assert (len(input_mask) == seq_length)
        assert (len(input_type_ids) == seq_length)
        (yield InputFeatures(unique_id=example.unique_id, tokens=tokens, input_ids=input_ids, input_mask=input_mask, input_type_ids=input_type_ids))
