...
def model_fn_builder(bert_config, init_checkpoint, learning_rate, num_train_steps, num_warmup_steps, use_tpu, use_one_hot_embeddings):
    ...
    def model_fn(features, labels, mode, params):
        ...
        if (mode == tf.estimator.ModeKeys.TRAIN):
            ...
            predictions = {'unique_ids': unique_ids, 'start_logits': start_logits, 'end_logits': end_logits}
...
def input_fn_builder(input_file, seq_length, is_training, drop_remainder):
    ...
    name_to_features = {'unique_ids': tf.FixedLenFeature([], tf.int64), 'input_ids': tf.FixedLenFeature([seq_length], tf.int64), 'input_mask': tf.FixedLenFeature([seq_length], tf.int64), 'segment_ids': tf.FixedLenFeature([seq_length], tf.int64)}
    if is_training:
