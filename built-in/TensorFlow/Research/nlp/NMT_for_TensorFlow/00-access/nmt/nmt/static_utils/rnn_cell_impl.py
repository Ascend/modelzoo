  ...
  def call(self, inputs, state, scope=None):
    ...
    Returns:
      A pair containing the new hidden state, and the new state (either a
        ...
        `state_is_tuple`).
    """
    with tf.name_scope('define_basicLSTM'):
        ...
        #mask = npu_ops.dropout_gen_mask(inputs, keep_prob)
        if os.environ.get('USE_NPU') in [ 'True', '1' ]:
            ct,ht,it,jt,ft,ot,tanhct = npu_ops.basic_lstm_cell(
                inputs, h, c, self._kernel, self._bias, keep_prob=1.0, forget_bias=self._forget_bias, state_is_tuple=self._state_is_tuple, activation='tanh')
            new_h = ht
            new_c = ct
            if self._state_is_tuple:
...
class DropoutWrapper(RNNCell):
  ...
  def __init__(self, cell, input_keep_prob=1.0, output_keep_prob=1.0,
    ...
    self._cell = cell
    if isinstance(cell, checkpointable.Trackable):
    ...
    if not self._variational_recurrent:
      def dropout(i, do_dropout, v):
        if not isinstance(do_dropout, bool) or do_dropout:
          # XXX
          if os.environ.get('USE_NPU') in [ 'True', '1' ]:
              my_dropout = npu_ops.dropout
          else:
              my_dropout = nn_ops.dropout
          return my_dropout(
...
class MultiRNNCell(RNNCell):
  ...
  def call(self, inputs, state, scope=None):
    ...
    self._cc_index = self._cc_index + 1
