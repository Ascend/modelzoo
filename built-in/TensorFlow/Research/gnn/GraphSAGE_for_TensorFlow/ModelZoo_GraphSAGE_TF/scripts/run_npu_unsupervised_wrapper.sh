...
python3 graphsage/unsupervised_train.py --train_prefix data/${DATASET}/${DATASET} --model ${MODEL} --base_log_dir outputs/${DATASET} --device npu --device_ids ${DEVICE_ID} --rank_size ${DEVICE_RANK}
if [ ${DEVICE_RANK} -eq 1 ] || [ ${DEVICE_ID} -eq 0 ]; then
    python3 eval_scripts/ppi_eval.py data/${DATASET}/${DATASET} outputs/${DATASET}/unsup-${DEVICE_RANK}p/${MODEL} test
fi
