...
class QMixModel(object):
    ...
    def __init__(self, model_info):
        ...
        flag_npu = model_info["use_npu"]

        if flag_npu and model_info.get("type", "actor") is "learner":
            print(model_info)
            ...
            custom_op = (
                session_config.graph_options.rewrite_options.custom_optimizers.add()
            )
            ...
            # above 3 and last 1 alter

            custom_op.parameter_map["use_off_line"].b = True
            
            if model_info["allow_mix_precision"] == True:
        ...
        with self.graph.as_default():
            ...
            self.ph_hidden_states_in = tf.placeholder(
                tf.float32, shape=(self.n_agents, self.rnn_hidden_dim), name="hidden_in"
            )
            ...
            self.ph_train_obs_len = tf.placeholder(
                tf.float32, shape=(self.n_agents*self.batch_size,), name="train_obs_len"
            )
        ...
        with self.graph.as_default():
            ...
            print("<<< hi_out_val_default shape: >>> ", np.shape(self.hi_out_val_default))
        ...
        #tf.io.write_graph(self.graph, 'logdir', 'qmix_train.pbtxt')
    ...
    def build_agent_net(self, inputs_obs, seq_max, obs_lengths, hidden_state_in):
        ...
        """
        fc1 = tf.layers.dense(
            inputs=inputs_obs, units=self.rnn_hidden_dim, activation=tf.nn.relu,
        )

        fc1 = tf.transpose(fc1, perm=[0, 2, 1, 3])
        logging.debug("fc1 before reshape: {}".format(fc1))
        # flag_0d, flag_1d, flag_2d, flag_3d = fc1.get_shape()

        # fc1 = tf.reshape(fc1, [-1, seq_max, self.rnn_hidden_dim])
        fc1 = tf.reshape(fc1, [1*self.n_agents, seq_max, self.rnn_hidden_dim])
        ...
        rnn_output = tf.reshape(
            ...
            rnn_output,
            [1, self.n_agents, seq_max, self.rnn_hidden_dim],
        )
        rnn_output = tf.transpose(rnn_output, perm=[0, 2, 1, 3])

        logging.debug("rnn_output before shape: {}".format(rnn_output))
        # rnn_output = tf.reshape(rnn_output, [-1, self.rnn_hidden_dim])
        rnn_output = tf.reshape(rnn_output, [1*1*self.n_agents, self.rnn_hidden_dim])
        logging.debug("rnn_output after  reshape: {}".format(rnn_output))
        # Tensor("explore_agent/transpose_1:0", shape=(1, 1, 2, 64), dtype=float32)
        # Tensor("explore_agent/Reshape_2:0", shape=(2, 64), dtype=float32)

        fc2_outputs = tf.layers.dense(
            inputs=rnn_output, units=self.n_actions, activation=None,
        )

        out_actions = tf.reshape(
            # fc2_outputs, (-1, self.n_agents, self.avail_action_num)
            fc2_outputs,
            (1 * seq_max, self.n_agents, self.avail_action_num),
        )
        logging.debug("out action: {}".format(out_actions))

        return out_actions, hidden_state_out

    def build_train_agent_net(self, inputs_obs, seq_max, obs_lengths, hidden_state_in):
        """build agent architecture,
        could work well among explorer & train with different sequence.
        """
        fc1 = tf.layers.dense(
        ...
        # fc1 = tf.reshape(fc1, [-1, seq_max, self.rnn_hidden_dim])
        fc1 = tf.reshape(fc1, [self.batch_size*self.n_agents, seq_max, self.rnn_hidden_dim])
        logging.debug("fc1 after reshape: {}".format(fc1))
        ...
        rnn_output = tf.reshape(
            # rnn_output, [-1, self.n_agents, seq_max, self.rnn_hidden_dim]
            rnn_output,
            [self.batch_size, self.n_agents, seq_max, self.rnn_hidden_dim],
        )
        rnn_output = tf.transpose(rnn_output, perm=[0, 2, 1, 3])

        logging.debug("rnn_output before shape: {}".format(rnn_output))
        # Tensor("target_agent/transpose_1:0", shape=(32, 301, 2, 64), dtype=float32)
        # rnn_output = tf.reshape(rnn_output, [-1, self.rnn_hidden_dim])
        rnn_output = tf.reshape(
            rnn_output,
            [self.batch_size*(self.fix_seq_length+1)*self.n_agents, self.rnn_hidden_dim])
        logging.debug("rnn_output after  reshape: {}".format(rnn_output))
        # Tensor("target_agent/Reshape_2:0", shape=(19264, 64), dtype=float32)
        ...
        out_actions = tf.reshape(
            # fc2_outputs, (-1, self.n_agents, self.avail_action_num)
            fc2_outputs,
            (self.batch_size * seq_max, self.n_agents, self.avail_action_num),
        )
        logging.debug("out action: {}".format(out_actions))
    ...
    def get_explore_actions(self, ep_batch, t_ep, t_env, test_mode):
        """explore action combination with selector"""
    ...
    #     :return:
    #     """
    #     logging.debug("inputs in gather: {}".format(inputs))
    #     len_0d, len_1d, len_2d, len_3d = inputs.get_shape().as_list()
    #     logging.debug(
    #         "len_0d-{}, len_1d-{}, len_2d-{}, len_3d-{}".format(
    #             len_0d, len_1d, len_2d, len_3d
    #         )
    #     )
    #     # inputs = tf.reshape(inputs, (-1, len_3d))
    #     print("\n\n >> input before: ", inputs)
    #     inputs = tf.reshape(inputs, (len_0d * len_1d * len_2d, len_3d))
    ...
    #     indices = tf.reshape(indices, [flag_0d * flag_1d * flag_2d, flag_3d])
    #
    #     idx_matrix = tf.tile(
    ...
    #
    #     inputs = tf.reshape(
    #         tf.boolean_mask(inputs, idx_mask), [flag_0d, flag_1d, flag_2d]
    #     )
    #     return inputs

    def _gather4d_on_dim3_fix(self, inputs, indices):
        """
        gather 4dim tensor into 3dim, same to the pytorch.gather + sequeeze(3) function.
        :param inputs:
        :param indices:
        ...
        print("\n\n >> input before: ", inputs)
        inputs = tf.reshape(
            inputs, (self.batch_size*self.n_agents*self.fix_seq_length, self.n_actions))
        print(">> input after: ", inputs, "\n\n")
        ...
        indices = tf.reshape(
            indices, [self.batch_size*self.n_agents*self.fix_seq_length, 1])
        print("indices after: ", indices, "\n\n")

        idx_matrix = tf.tile(
            tf.expand_dims(tf.range(0, self.n_actions, dtype=indices.dtype), 0),
            [self.batch_size*self.n_agents*self.fix_seq_length, 1]
        )
        indices_t = tf.transpose(indices)
        idx_mask = tf.equal(idx_matrix, tf.transpose(indices_t))

        inputs = tf.reshape(
            tf.boolean_mask(inputs, idx_mask),
            [self.batch_size, self.fix_seq_length, self.n_agents]
        )
        return inputs
    ...
    def _build_mix_net2(self, agent_qs, states):
        ...
        def hyper_w1(hyper_w1_input):
            ...
            with tf.variable_scope("hyper_w1"):
                hw0 = tf.layers.dense(
                    inputs=hyper_w1_input, units=hypernet_embed, activation=tf.nn.relu
                )
                hw1 = tf.layers.dense(
                    inputs=hw0, units=self.embed_dim * self.n_agents, activation=None
                )
                return hw1
        ...
        def hyper_w_final(hyper_w_final_input):
            ...
            with tf.variable_scope("hyper_w_final"):
                ...
                hw_f1 = tf.layers.dense(
                    inputs=hw_f0, units=self.embed_dim, activation=None
                )
                return hw_f1
        ...
        def hyper_b1(state_input):
            ...
            with tf.variable_scope("hyper_b1"):
                return tf.layers.dense(
                    inputs=state_input, units=self.embed_dim, activation=None
                )

        def val(state_input):
            """V(s) instead of a bias for the last layers"""
            with tf.variable_scope("val_for_bias"):
                val0 = tf.layers.dense(
                    inputs=state_input, units=self.embed_dim, activation=tf.nn.relu
                )
        ...
        states_reshaped = tf.reshape(
            states, (self.batch_size * self.fix_seq_length, self.state_dim)
        )

        # agent_qs_reshaped = tf.reshape(agent_qs, (-1, 1, self.n_agents))
        agent_qs_reshaped = tf.reshape(
            agent_qs, (self.batch_size * self.fix_seq_length, 1, self.n_agents)
        )
        ...
        w1_reshaped = tf.reshape(
            w1, (self.batch_size * self.fix_seq_length, self.n_agents, self.embed_dim)
        )

        b1_reshaped = tf.reshape(
            b1, (self.batch_size * self.fix_seq_length, 1, self.embed_dim)
        )

        # print("tf.matmul(agent_qs_reshaped, w1_reshaped), b1_reshaped")
        # print(tf.matmul(agent_qs_reshaped, w1_reshaped), b1_reshaped)
        print(">>> tf.matmul: agent_qs_reshaped, w1_reshaped into matmul")
        print(agent_qs_reshaped, w1_reshaped)
        ...
        w_final_reshaped = tf.reshape(
            w_final, (self.batch_size * self.fix_seq_length, self.embed_dim, 1)
        )

        # state-dependent bias
        # v = tf.reshape(val(states_reshaped), (-1, 1, 1))
        # v = tf.reshape(val(states_reshaped), (self.batch_size*self.fix_seq_length, 1, 1))
        v = tf.reshape(
            val(tf.cast(states_reshaped, tf.float16)),
            (self.batch_size * self.fix_seq_length, 1, 1),
        )

        # compute final output
        print(">>> hidden, w_final_reshaped into matmul")
        print(hidden, w_final_reshaped)
        # print(tf.matmul(hidden, w_final_reshaped), v)

        # matmul NOT support shape contains 1 within the 2-d data
        # y = tf.math.add(tf.matmul(hidden, w_final_reshaped), v)
        y = tf.math.add(
            tf.matmul(
                tf.cast(hidden, tf.float16), tf.cast(w_final_reshaped, tf.float16)
            ),
            tf.cast(v, tf.float16),
        )

        # reshape and return
        # q_tot = tf.reshape(y, (bs, -1, 1))
        # q_tot = tf.cast(tf.reshape(y, (bs, -1, 1)), tf.float32)
        q_tot = tf.cast(tf.reshape(y, (self.batch_size, self.fix_seq_length, 1)), tf.float32)
        ...
        with self.graph.as_default():
            ...
            with tf.variable_scope("soft_replacement"):
                self.agent_train_replace_op = [
                    tf.assign(t, e)
                    for t, e in zip(_target_agent_paras, _eval_agent_paras)
                ]

                self.agent_explore_replace_op = [
                    tf.assign(t, e)
                    for t, e in zip(self._explore_paras, _eval_agent_paras)
                ]
            ...
            mac_out = tf.reshape(
                ...
                [self.batch_size, self.fix_seq_length + 1, self.n_agents, self.n_actions],
            )
            logging.debug("mac_out: {}".format(mac_out))
            chosen_action_qvals = self.gather_custom(
                mac_out[:, :-1], self.ph_actions
            )
            ...
            logging.debug("target_mac_out: {}".format(target_mac_out))
            ...
            logging.debug(
                "indices:{}, mask_val:{}, target mac out:{}".format(
            ...
            if self.use_double_q:
                ...
                target_max_qvals = self.gather_custom(
                    target_mac_out, cur_max_actions
                )
            else:
            ...
            with tf.variable_scope("eval_mixer"):
                self.q_tot = self._build_mix_net2(
                    chosen_action_qvals, self.ph_train_states
                )

            with tf.variable_scope("target_mixer"):
                q_tot_tmp = self._build_mix_net2(
                    target_max_qvals, self.ph_train_target_states
                )
            ...
            with tf.variable_scope("soft_replacement"):
                self.mix_train_replace_op = [
                    tf.assign(t, e) for t, e in zip(_target_mix_paras, _eval_mix_paras)
                ]

            self._print_trainable_var_name(
                _eval_mix_paras=_eval_mix_paras, _target_mix_paras=_target_mix_paras
            )
            ...
            self.loss = tf.reduce_sum(masked_td_error ** 2) / tf.reduce_sum(
                self.ph_mask
            )
            ...
            capped_gvs = [
                (
                    grad
                    if grad is None
                    else tf.clip_by_norm(grad, clip_norm=self.grad_norm_clip),
                    var,
                )
    ...
    def save_explore_agent_weights(self, save_path):
        ...
        self.explore_saver.save(self.sess, save_path=save_path, write_meta_graph=False)
