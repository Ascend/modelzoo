...
from xt.benchmark.analyze.tensorboarder import clean_board_dir
...
from xt.benchmark.tools.evaluate_xt import parse_benchmark_args
from xt.benchmark.tools.evaluate_xt import read_train_records, read_train_event_id


def display_rewards(args, stage):
    """utils for display, support multi & single config file"""
...
def parse_xt_train_config(yaml_obj):
    """utils for parse xt config file"""
...
def handle_once_local_data_record(
        case_paras, use_index, stage="eval", clear_tensorboard=True
):
...
def write2board(stage, record_dict, use_index, case_tb_dir):
    ...
    summary = Summary(logdir_root=bm_conf.default_tb_path, fixed_path=case_tb_dir)
