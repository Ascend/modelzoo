...
class NAMLModel(BaseModel):
    ...
    def _build_userencoder(self, newsencoder):
        ...
        user_present = AttLayer2(200, seed=self.seed)(
            click_news_presents
        )
    ...
    def _build_newsencoder(self, embedding_layer):
        ...
        news_repr = AttLayer2(200, seed=self.seed)(
            concate_repr
        )
    ...
    def _build_titleencoder(self, embedding_layer):
        ...
        """
        hparams = self.hparams
        sequences_input_title = keras.Input(shape=(30,), dtype="int32")
        embedded_sequences_title = embedding_layer(sequences_input_title)

        y = layers.Dropout(0.2)(embedded_sequences_title)
        y = layers.Conv1D(
            400,
            3,
            activation="relu",
        ...
        pred_title = AttLayer2(200, seed=self.seed)(y)
    ...
    def _build_bodyencoder(self, embedding_layer):
        ...
        """
        hparams = self.hparams
        sequences_input_body = keras.Input(shape=(50,), dtype="int32")
        embedded_sequences_body = embedding_layer(sequences_input_body)

        y = layers.Dropout(0.2)(embedded_sequences_body)
        y = layers.Conv1D(
            400,
            3,
            activation="relu",
        ...
        y = layers.Dropout(0.2)(y)
        pred_body = AttLayer2(200, seed=self.seed)(y)
        pred_body = layers.Reshape((1, 400))(pred_body)
    ...
    def _build_vertencoder(self):
        ...
        pred_vert = layers.Reshape((1, 400))(pred_vert)
    ...
    def _build_subvertencoder(self):
        ...
        pred_subvert = layers.Reshape((1, 400))(pred_subvert)
    ...
    def _build_naml(self):
        ...
        his_input_title = keras.Input(
            shape=(50, 30), dtype="int32"
        )
        his_input_body = keras.Input(
            shape=(50, 50), dtype="int32"
        )
        his_input_vert = keras.Input(shape=(50, 1), dtype="int32")
        his_input_subvert = keras.Input(shape=(50, 1), dtype="int32")

        pred_input_title = keras.Input(
            shape=(4 + 1, 30), dtype="int32"
        )
        pred_input_body = keras.Input(
            shape=(4 + 1, 50), dtype="int32"
        )
        pred_input_vert = keras.Input(shape=(4 + 1, 1), dtype="int32")
        pred_input_subvert = keras.Input(shape=(4 + 1, 1), dtype="int32")

        pred_input_title_one = keras.Input(
            shape=(1, 30,), dtype="int32"
        )
        pred_input_body_one = keras.Input(shape=(1, 50,), dtype="int32")
