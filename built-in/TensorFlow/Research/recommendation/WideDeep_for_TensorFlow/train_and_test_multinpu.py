...
def test_train_eval(config):
    ...
    ds_eval = create_dataset(data_path, train_mode=False, epochs=epochs + 1, is_tf_dataset=config.is_tf_dataset,
                             batch_size=batch_size, rank_id=get_rank(), rank_size=get_group_size())
    ...
    model.train(epochs, ds_train, callbacks=[TimeMonitor(ds_train.get_dataset_size()), eval_callback, callback, ckpoint_cb])
