...
def _get_h5_dataset(dir, train_mode=True, epochs=1, batch_size=1000):
    data_para = {
    ...
    ds = de.GeneratorDataset(lambda: _iter_h5_data(),
                             ["ids", "weights", "labels"])
...
def _get_tf_dataset(dir, train_mode=True, epochs=1, batch_size=1000,
    ...
    if rank_size is not None and rank_id is not None:
        ds = de.TFRecordDataset(dataset_files=dataset_files, shuffle=shuffle, schema=schema, num_parallel_workers=8, num_shards=rank_size, shard_id=rank_id, shard_equal_rows=True)
    else:
...
def create_dataset(dir, train_mode=True, epochs=1, batch_size=1000,
                   is_tf_dataset=True, line_per_sample=1000, rank_size=None, rank_id=None):
    
    if is_tf_dataset:
        return _get_tf_dataset(dir, train_mode, epochs, batch_size,
        ...
        return _get_h5_dataset(dir, train_mode, epochs, batch_size)
