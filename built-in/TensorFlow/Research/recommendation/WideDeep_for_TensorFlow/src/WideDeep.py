...
from mindspore import Model, Tensor, Parameter, ParameterTuple, context
...
from mindspore.nn.optim import Momentum, Adam, FTRL
    ...
    for _i in range(len(vars)):
        ...
        if key not in var_map.keys():
            if init_method in ['random', 'uniform']:
                ...
                var_map[key] = Parameter(Tensor(np.random.normal(loc=0.0, scale=0.01, size=shape).astype(dtype=np_type)), name=key)
    return var_map
...
class DenseLayer(nn.Cell):
    ...
    Args:
    """
    def __init__(self, input_dim, output_dim, weight_bias_init, act_str, keep_prob=0.7, scale_coef=1.0, convert_dtype=True):
...
class WideDeepModel(nn.Cell):
    ...
    def __init__(self, config):
        ...
        self.deep_layer_dims_list = config.deep_layers_dim
        ...
        # for noah test
        # self.dense_layer_6 = DenseLayer(self.all_dim_list[5],
        #                                 self.all_dim_list[6],
        #                                 self.weight_bias_init,
        #                                 self.deep_layer_act, convert_dtype=True)
    ...
    def construct(self, id_hldr, wt_hldr):
        ...
        # deep_out = self.dense_layer_6(deep_in)
...
class TrainStepWrap(nn.Cell):
    ...
    def __init__(self, network, config, sens=1000.0):
        ...
        self.optimizer_w = FTRL(learning_rate=config.ftrl_lr, params=self.weights_w,
                                l1=1e-8, l2=1e-8, initial_accum=1.0, loss_scale=sens)

        #self.optimizer_d = ProximalAdagrad(self.weights_d)
        self.optimizer_d = Adam(self.weights_d, learning_rate=config.adam_lr, eps=1e-8,
                                loss_scale=sens)
