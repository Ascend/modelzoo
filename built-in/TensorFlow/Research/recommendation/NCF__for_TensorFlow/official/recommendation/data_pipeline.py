...
class DatasetManager(object):
  ...
  def get_dataset(self, batch_size, epochs_between_evals):
    ...
    if self._stream_files:
      ...
      if self._is_training and rank_size > 1:
...
class BaseDataConstructor(threading.Thread):
  ...
  def __init__(
    ...
    self.train_batches_per_epoch = self._count_batches(
        self._elements_in_epoch, train_batch_size, batches_per_train_step, True)
    ...
    self.eval_batches_per_epoch = self._count_batches(
        self._eval_elements_in_epoch, eval_batch_size, batches_per_eval_step, False)
  ...
  @staticmethod
  def _count_batches(example_count, batch_size, batches_per_step, istraining):
    """Determine the number of batches, rounding up to fill all devices."""
    x = (example_count + batch_size - 1) // batch_size
    if istraining:
    	return (x - 1) // batches_per_step * batches_per_step
  ...
  def make_input_fn(is_training):
    ...
    def input_fn(params):
      ...
      users = tf.random.uniform([batch_size, 1],
                                ...
                                minval=0,
                                maxval=num_users,
                                seed=0)                     #set random seed fix
      items = tf.random.uniform([batch_size, 1],
                                dtype=tf.int32,
                                minval=0,
                                maxval=num_items,
                                seed=1)                      #set random seed fix
      ...
      if is_training:
        valid_point_mask = tf.cast(
            tf.random.uniform([batch_size, 1],
                              ...
                              minval=0,
                              maxval=2,
                              seed=2), tf.bool)               #set random seed fix
        labels = tf.cast(
            tf.random.uniform([batch_size, 1],
                              dtype=tf.int32,
                              minval=0,
                              maxval=2,
                              seed=3), tf.bool)               #set random seed fix
        data = {
            movielens.USER_COLUMN: users,
            movielens.ITEM_COLUMN: items,
            rconst.VALID_POINT_MASK: valid_point_mask,
        ...
        dupe_mask = tf.cast(
            tf.random.uniform([batch_size, 1],
                              ...
                              minval=0,
                              maxval=2,
                              seed=4), tf.bool)                #set random seed fix
        data = {
            movielens.USER_COLUMN: users,
            movielens.ITEM_COLUMN: items,
