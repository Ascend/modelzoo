...
class VAE:
    def __init__(self,
        ...
        self.checkpoint_dir = checkpoint_dir if self.rank_id == 0 else None
    ...
    def _create_dataset(self, train_data, batch_size_train, encoder_dims):
        generator, self.n_batch_per_train = self.batch_iterator(train_data,
                                                                None,
                                                                batch_size_train,
                                                                thread_idx=self.rank_id,
                                                                thread_num=self.rank_size)
    ...
    def broadcast_global_variables(self, root_rank):
    ...
    def _build_optimizer(self, loss):
        ...
        return npu_opt.minimize(loss, global_step=tf.train.get_or_create_global_step())
    ...
    def batch_iterator(self, data_input, data_true=None, batch_size=500, thread_idx=0, thread_num=1):
        ...
        global_batch_size = batch_size * self.rank_size
    ...
    def train(
        ...
        if self.rank_size > 1:
            self.session.run(self.broadcast_global_variables(0))

        num_workers = self.rank_size
        for epoch in range(1, n_epochs + 1):
            ...
            if (epoch % validation_step == 0 or epoch == n_epochs) and self.rank_id == 0:
                init_time = time.time()
