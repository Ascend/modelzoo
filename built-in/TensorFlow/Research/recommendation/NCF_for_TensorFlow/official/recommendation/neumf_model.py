...
def neumf_model_fn(features, labels, mode, params):
  ...
  if mode == tf.estimator.ModeKeys.EVAL:
    ...
    if params["accelerator"] == "1980":
        print("use npudistributedoptimizer")
        optimizer=NPUDistributedOptimizer(optimizer)# for npu
...
def construct_model(user_input, item_input, params):
  ...
  # for matmul
  #from official.recommendation import tf_keras_layers_dense_redefined

  #logits = tf_keras_layers_dense_redefined.Dense(
  #    1, activation=None, kernel_initializer="lecun_uniform",
  #    name=movielens.RATING_COLUMN)(predict_vector)
