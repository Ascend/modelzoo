...
do
    ...
    ${bind_core} python3 graphsage/supervised_train.py \
	    --train_prefix $data_path/ppi/ppi \
		--model meanpool \
		--base_log_dir ${cur_path}/output/$ASCEND_DEVICE_ID/ckpt \
		--device npu \
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
Runtime=`cat $cur_path/output/0/train_0.log | grep "Runtime" | grep -v "supervised_train.py" | awk '{print $2}'`
FPS=`echo "scale=5;1000 / $Runtime * 512"|bc`
#打印，不需要修改
echo "Final Performance images/sec : $FPS"

#输出训练精度,需要模型审视修改
train_accuracy=`cat $cur_path/output/0/train_0.log|grep "f1_micro="|grep "validation"|grep -v "supervised_train.py"|awk -F "=" '{print $2}'|awk -F "," '{print $1}'`
#打印，不需要修改
...
grep 'loss=' $cur_path/output/0/train_0.log|grep -v 'supervised_train.py'|awk '{print $3}'|sed 's/,//g'|sed 's/loss=//g' >> $cur_path/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
