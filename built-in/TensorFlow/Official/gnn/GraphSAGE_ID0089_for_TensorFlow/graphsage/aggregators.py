...
class SeqStaticAggregator(SeqAggregator):
    """ Aggregates via a standard static LSTM.
    """
    def _call(self, inputs):
        self_vecs, neigh_vecs = inputs
        ...
        with tf.variable_scope(self.name) as scope:
            try:
                rnn_outputs, rnn_states = tf.nn.static_rnn(
                    self.cell, [neigh_vecs[:, t, :] for t in range(neigh_vecs.shape[1])],
                    initial_state=initial_state, dtype=tf.float32,
                    sequence_length=length)
                rnn_outputs = tf.concat([output[:, None] for output in rnn_outputs], axis=1)
            except ValueError:
                scope.reuse_variables()
                rnn_outputs, rnn_states = tf.nn.static_rnn(
                    self.cell, [neigh_vecs[:, t, :] for t in range(neigh_vecs.shape[1])],
