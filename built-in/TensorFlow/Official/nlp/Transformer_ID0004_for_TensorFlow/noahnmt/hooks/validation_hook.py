...
class ValidationHook(tf.train.SessionRunHook):
  ...
  def __init__(self, estimator=None, input_fn=None, batch_size=None,
               ...
               early_stopping_metric_delta=None, name=None, **kargs):
    ...
    if (self.eval_keep_best_n > 0 and 
      ...
      if len(self._all_values) <= self.eval_keep_best_n:
        ...
        sorted_values = sorted(
            self._all_values, 
            key=lambda x: x[1][self.eval_keep_best_metric], 
        ...
        best_n = sorted_values[:self.eval_keep_best_n]
        with tf.gfile.GFile(os.path.join(output_dir, 'checkpoint'), 'w') as f:
          best_name = "model.ckpt-{}".format(best_n[0][0])
          f.write("model_checkpoint_path: \"%s\"\n" % best_name)
          for i, (step, values) in enumerate(best_n):
            ckpt_name = "model.ckpt-{}".format(step)
