...
# from noahnmt import global_vars
...
class TrainSampleHook(TrainingHook):
  ...
  def before_run(self, _run_context):
    ...
    if self._should_trigger:
      fetches = {
          ...
          "target_words": self._pred_dict["features.target_tokens"],
          "target_len": self._pred_dict["features.target_len"],
          "source_words": self._pred_dict["features.source_tokens"],
          "source_len": self._pred_dict["features.source_len"]
      }
  ...
  def after_run(self, _run_context, run_values):
    ...
    for result in result_dicts:
      ...
      sourcce_len = result["source_len"]
      predicted_slice = result["predicted_tokens"][:target_len - 1]
      target_slice = result["target_words"][1:target_len]
      source_slice = result["source_words"][:sourcce_len-1]
      result_str += "Source: " + self._target_delimiter.encode("utf-8").join(
          source_slice).decode("utf-8") + "\n"
      result_str += "Trans: " + self._target_delimiter.encode("utf-8").join(
          predicted_slice).decode("utf-8") + "\n"
      result_str += "Refs: " + self._target_delimiter.encode("utf-8").join(
...
class PrintModelAnalysisHook(TrainingHook):
  ...
  def begin(self):
    ...
    if self.is_chief:
      ...
      opts["output"] = "file:outfile=%s" % os.path.abspath(self._filename)
      tf.profiler.profile(
...
class VariableRestoreHook(TrainingHook):
  ...
  @staticmethod
  def default_params():
    return {"prefix": None, "checkpoint_path": ""}

  def begin(self):
    # get the name-value pairs in checkpoint
    ...
    for name, shape in ckpt_vars:
      if not self.params["prefix"] or name.startswith(self.params["prefix"]):
    ...
    restore_map = {var.name[:-2]: var for var in variables if var.name[:-2] in values}

    tf.logging.info("Restoring variables: \n%s" %
                    yaml.dump({k: v.get_shape().as_list()
                               for k, v in restore_map.items()}, width=tf.int32.max))
  ...
  #   target_names = [varname_in_checkpoint(_.op.name[:-2]) for _ in variables]
...
# from noahnmt.hooks.prune_hook import GradualPruneHook


@registry.register_class
class SaveTensorHook(TrainingHook):
  """A hook to capture metadata for a single step.
  Useful for performance debugging. It performs a full trace and saves
  run_metadata and Chrome timeline information to a file.
  ...
  """
  def __init__(self, params, model_dir, run_config):
    super(SaveTensorHook, self).__init__(params, model_dir, run_config)
    self._active = False
    self._done = False
    self._global_step = None
    self._output_dir = os.path.abspath(self.model_dir)
  ...
  def before_run(self, _run_context):
    fetches = graph_utils.get_dict_from_collection("SAVE_TENSOR")
    return tf.train.SessionRunArgs([fetches, self._global_step])
  ...
  def after_run(self, _run_context, run_values):
    ...
    out_filename = os.path.join(self._output_dir, "step%d."%step + self.params["filename"])
    np.savez(out_filename, **result_dict)
