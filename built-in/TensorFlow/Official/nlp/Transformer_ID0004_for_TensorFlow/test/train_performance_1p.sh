...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
FPS=`grep "INFO:tensorflow:" $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|grep 'words/sec'|tr -d 'k'|awk 'END {print 1000*$NF}'`
#打印，不需要修改
...
grep "INFO:tensorflow:"  $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|grep loss |tr -d ','|awk -F '=' '{print $2}' |awk '{print $1}'> $cur_path/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
