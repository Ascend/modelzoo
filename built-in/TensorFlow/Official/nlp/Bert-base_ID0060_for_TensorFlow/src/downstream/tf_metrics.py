...
from metrics_impl import _streaming_confusion_matrix
...
def pr_re_fbeta(cm, pos_indices, beta=1):
    ...
    cm_mask = np.ones([num_classes, num_classes], dtype=np.float32)
...
def metrics_from_confusion_matrix(cm, pos_indices=None, average='micro',
    ...
    if average == 'micro':
        ...
        for idx in pos_indices:
            ...
            recalls.append(re)
            fbetas.append(fbeta)
            cm_mask = np.zeros([num_classes, num_classes],dtype=np.float32)
            cm_mask[idx, :] = 1
