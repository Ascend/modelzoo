...
flags.DEFINE_bool("debug_cce", False, "Whether dump cce")
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      logging_hook = tf.train.LoggingTensorHook(
        {"loss":total_loss,"masked_lm_loss":masked_lm_loss,"next_sentence_loss":next_sentence_loss,"global_step":tf.train.get_global_step()},every_n_iter=FLAGS.iterations_per_loop)
      ##################NPU_modify end#############################
...
class _LogSessionRunHook(tf.train.SessionRunHook):
    ...
    def after_run(self, run_context, run_values):
        if not run_values.results[0]:
            ...
            print('Apply gradients, loss scale value=%d' % run_value.results[1], flush=True)
##################NPU_modify end#############################
...
def main(_):
  ...
  if FLAGS.do_train:
    ...
    ##################NPU_modify start#############################
    estimator.train(input_fn=train_input_fn, max_steps=FLAGS.num_train_steps, hooks=npu_hooks_append(hooks_list=training_hooks))
