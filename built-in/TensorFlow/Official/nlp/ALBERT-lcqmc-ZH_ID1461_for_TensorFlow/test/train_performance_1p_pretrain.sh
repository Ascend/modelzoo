...
# nohup python3 run_pretraining.py   --input_file=${data_path}/tf*.tfrecord   --do_train=$do_train   --do_eval=$do_eval  --train_batch_size=$train_batch_size  \
#     --max_predictions_per_seq=$max_predictions_per_seq --bert_config_file=$ckpt_path/albert_config_tiny.json --max_seq_length=$max_seq_length  --learning_rate=$learning_rate  --num_train_steps=$num_train_steps \
#     --num_warmup_steps=$num_warmup_steps --save_checkpoints_steps=$save_checkpoints_steps --output_dir=$output_dir --init_checkpoint=$ckpt_path/albert_model.ckpt  > $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log 2>&1 &
