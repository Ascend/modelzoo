...
def get_iterator(src_dataset,
  ...
  if os.environ["use_npu_distribution"] == "true":
      rank_size = int(os.getenv('RANK_SIZE').strip())
      # rank_id = int(os.getenv('RANK_INDEX').strip())
      rank_id = int(os.getenv('DEVICE_ID').strip())
      # local_rank = rank_id * 8 + device_id
      print('RANK_SIZE=', rank_size, ' RANK_ID=', rank_id)
      src_tgt_dataset = src_tgt_dataset.shard(rank_size, rank_id)
  else:
  ...
  if os.environ["data_shuffle"] == "true":
