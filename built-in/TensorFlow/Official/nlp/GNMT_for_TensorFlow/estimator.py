  ...
  def build_graph_npu(self, features, labels, mode, params):
      ...
      if mode == tf.contrib.learn.ModeKeys.INFER:
          ...
          model = gnmt_model.GNMTModel(
              hparams, mode=mode, features=features)
          # sync training.
...
def train_fn(hparams):
  ...
  if os.environ["use_npu"] == "true":
      ...
      dump_config = DumpConfig(enable_dump=True, dump_path="/home/chujun/output", dump_step="0", dump_mode="all")
