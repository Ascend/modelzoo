...
def train():
    ...
    for epoch in range(0, n_epoch + 1):
        ...
        log = "[*] Epoch: [%2d/%2d] final_time: %4.4fs, final_d_loss: %.8f final_g_loss: %.8f" % (epoch, n_epoch, time.time() - epoch_time, total_d_loss / n_iter,
                                                                                total_g_loss / n_iter)
        print(log)
        ...
        if (epoch != 0) and (epoch % 10 == 0):
            ...
            save_dir = os.path.join(project_path, 'ckpt_gpu')
            if not os.path.exists(save_dir):
                os.makedirs(save_dir)
            save_ckpt = os.path.join(save_dir, 'srgan_{}'.format(tl.global_flag['mode']) + 'model.ckpt')
            saver.save(sess, save_ckpt)
            print('ckpt has been saved')
...
def evaluate():
    ...
    ###========================== DEFINE MODEL ============================###
    imid = 64  # 0: 浼侀箙  81: 铦磋澏 53: 楦  64: 鍙ゅ牎
    valid_lr_img = valid_lr_imgs[imid]
    valid_hr_img = valid_hr_imgs[imid]
    # valid_lr_img = get_imgs_fn('test.png', 'data2017/')  # if you want to test your own image
    valid_lr_img = (valid_lr_img / 127.5) - 1  # rescale to 锛伙紞1, 1]
