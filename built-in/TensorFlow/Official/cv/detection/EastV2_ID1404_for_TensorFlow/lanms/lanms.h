...
namespace lanms {
	...
	class PolyMerger {
		public:
			...
			Polygon normalize_poly(
					const Polygon &ref,
					const Polygon &p) {
                // find correspoding matching order of p with respect to ref
                // in which, vectix of p is most close to ref.
	...
	std::vector<Polygon> standard_nms(std::vector<Polygon> &polys, float iou_threshold) {
        ...
        /**TODO: nms_with_bbox is faster. 
        // suppress polys with large area. Suppression the 1st time.
        std::vector<size_t> suppressed(n, 0);
        std::vector<size_t> keep;
        for(size_t _i=1; _i<indices.size(); _i++){
            size_t i = indices[_i];
            if (suppressd[i]==1)
                continue;
            keep.emplace_back(i);
        ...
        **/
        // After first suppression, then suppress 2nd time.
		std::vector<size_t> keep;
		while (indices.size()) {
        ...
        finish_t = clock();
        double duration = double(finish_t - start_t)/CLOCKS_PER_SEC*1000.0;
        //printf("standard-nms with %d polygons, takes: %f ms.\n", n, duration); 
		return ret;
	}
