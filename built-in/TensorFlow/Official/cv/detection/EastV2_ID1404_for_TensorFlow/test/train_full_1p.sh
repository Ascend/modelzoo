...
start=$(date +%s)
nohup python3 -u multigpu_train.py > $cur_path/test/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log 2>&1 &
wait
end=$(date +%s)
e2e_time=$(( $end - $start ))
...
#最后一个迭代loss值，不需要修改
ActualLoss=`tail -n 5 $cur_path/test/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt|awk -F " " '{number[$1]=$1}END{for(i in number){print number[i]|"sort -n|head -n 1"}}'`
