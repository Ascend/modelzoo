...
export RANK_TABLE_FILE=/autotest/c00314821/fast-rcnn-1207/npu_config/8p.json
...
su HwHiAiUser -c "/usr/local/Ascend/toolkit/bin/adc --host 127.0.0.1:22118 --log \"SetLogLevel(0)[error]\""
su HwHiAiUser -c "/usr/local/Ascend/toolkit/bin/adc --host 127.0.0.1:22118 --log \"SetLogLevel(0)[error]\" --device 4"
...
mkdir -p $TMP_PATH
cp exec_main_8p.sh $TMP_PATH/
cp profSetEnvForNoDocker.sh $TMP_PATH/
cd $TMP_PATH
# profiling
#source profSetEnvForNoDocker.sh $RANK_ID
# run
bash exec_main_8p.sh $RANK_ID $RANK_SIZE $BASE_PATH 2>&1 | tee train_$RANK_ID.log &
cd -
done
