...
def build_model_graph(features, labels, is_training, params):
  ...
  if is_training:
    ...
    # add pixel scoring here
    # high confidence mask
    probs = tf.nn.sigmoid(mask_outputs)

    # transform mask logits 2 confidence mask, not well
    inst_hcm = probs > 0.2
    inst_hcm = tf.cast(inst_hcm, dtype=probs.dtype)

    # high confidence value
    inst_hcv = tf.math.reduce_sum(inst_hcm * probs, axis=[2, 3])

    # number of hcm
    inst_hcm_num = tf.math.reduce_sum(inst_hcm, axis=[2, 3])

    # instance pixel score (batch_size, num_box)
    pixel_score = inst_hcv / inst_hcm_num

    # update scores
    model_outputs['detection_mask_scores'] = pixel_score * model_outputs['detection_scores']
