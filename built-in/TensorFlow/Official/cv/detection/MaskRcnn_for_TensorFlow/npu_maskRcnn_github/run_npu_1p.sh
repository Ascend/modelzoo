...
rm -rf training/*
# set env
export PYTHONPATH=/usr/local/Ascend/ops/op_impl/built-in/ai_core/tbe/:../:/opt/npu/x00558890/item_npu3/thirdlib:/usr/local/Ascend/fwkacllib/python/site-packages
...
PATH=$PATH:$HOME/bin
export PATH=$PATH:/usr/local/Ascend/fwkacllib/ccec_compiler/bin:$PATH:/usr/local/Ascend/fwkacllib/bin:/usr/local/Ascend/toolkit/bin
...
do
echo
#su HwHiAiUser -c "adc --host 127.0.0.1:22118 --log \"SetLogLevel(0)[debug]\" --device "$RANK_ID
