...
{
    if (argc <= 1) {
        LogWarn << "Please input image path, such as './alexnet /path/to/jpeg_image_dir'.";
        return APP_ERR_OK;
    }
    ...
    initParam.modelPath = "../models/alexnet_tf_aipp.om";
    auto alexnet = std::make_shared<AlexNetClassify>();
    ret = alexnet->Init(initParam);
    if (ret != APP_ERR_OK) {
    ...
    double fps = 1000.0 * imgFiles.size() / alexnet->GetInferCostTimeMs();
