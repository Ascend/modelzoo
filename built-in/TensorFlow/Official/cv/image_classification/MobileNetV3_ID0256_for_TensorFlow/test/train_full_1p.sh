...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="MobileNetV3_ID0256_for_TensorFlow"
#训练epoch
train_epochs=20
#训练step
train_steps=200
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
TrainingTime=`grep "ms/step" $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | awk -F "ms" '{print $1}' | awk -F " " 'END {print $5}'`
FPS=`awk 'BEGIN {printf "%.2f\n",'1000'*'${batch_size}'/'${TrainingTime}'}'`
#打印，不需要修改
echo "Final Performance images/sec : $FPS"

#输出训练精度,需要模型审视修改
train_accuracy=`grep "accuracy:" $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | tail -1 | awk -F " " '{print $11}'`
#打印，不需要修改
...
cat $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | grep -Eo "loss: [0-9]*\.[0-9]*" > $cur_path/output/$ASCEND_DEVICE_ID/${CaseName}_loss.txt
#最后一个迭代loss值，不需要修改
