...
export PYTHONPATH=$cur_path/../automl/:$PYTHONPATH
export RANK_SIZE=1
export JOB_ID=10087
...
do
    ...
    nohup python3  run_example.py > ${cur_path}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
done 
wait
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
step_time=`grep 'INFO loss' $cur_path/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log|awk -F 'sec' '{print $1}'|tr -d '('|awk 'NR>2'|awk '{print $NF}'|awk '{sum+=$1} END {print  1000*sum/NR/10}'`
