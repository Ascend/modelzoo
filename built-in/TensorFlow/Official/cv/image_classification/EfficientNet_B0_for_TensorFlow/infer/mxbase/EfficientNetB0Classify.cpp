...
#include "EfficientNetB0Classify.h"
...
APP_ERROR EfficientNetB0Classify::Init(const InitParam &initParam)
{
...
APP_ERROR EfficientNetB0Classify::DeInit()
{
    dvppWrapper_->DeInit();
    model_->DeInit();
    post_->DeInit();
...
APP_ERROR EfficientNetB0Classify::ReadImage(const std::string &imgPath, cv::Mat &imgMat)
{
...
APP_ERROR EfficientNetB0Classify::Resize(const cv::Mat &srcMat, cv::Mat &dstMat)
{
...
APP_ERROR EfficientNetB0Classify::CvMatToTensorBase(const cv::Mat &imgMat, MxBase::TensorBase &tensorBase)
{
...
APP_ERROR EfficientNetB0Classify::Crop(const cv::Mat &srcMat, cv::Mat &dstMat)
{
    static constexpr float centralFraction = 0.875;
    uint32_t minSize = std::min(srcMat.cols, srcMat.rows) * centralFraction;
    uint32_t amountCropHeight = srcMat.rows - minSize + 1;
    uint32_t amountCropWidth = srcMat.cols - minSize + 1;
    uint32_t x0 = amountCropWidth / 2;
    uint32_t y0 = amountCropHeight / 2;
    cv::Rect rectOfImg(x0, y0, minSize, minSize);
    dstMat = srcMat(rectOfImg).clone();
    return APP_ERR_OK;
}

APP_ERROR EfficientNetB0Classify::Inference(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR EfficientNetB0Classify::PostProcess(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR EfficientNetB0Classify::Process(const std::string &imgPath)
{
    cv::Mat image;
    ...
    cv::Mat cropImage;
    ret = Crop(image, cropImage);
    if (ret != APP_ERR_OK) {
        LogError << "Crop failed, ret=" << ret << ".";
        return ret;
    }
    cv::Mat resizeImage;
    ret = Resize(cropImage, resizeImage);
    if (ret != APP_ERR_OK) {
...
std::string EfficientNetB0Classify::FormatResultFile(const std::string &imgPath)
{
...
APP_ERROR EfficientNetB0Classify::SaveResult(const std::string &imgPath, const std::vector<std::vector<MxBase::ClassInfo>> &batchClsInfos)
{
