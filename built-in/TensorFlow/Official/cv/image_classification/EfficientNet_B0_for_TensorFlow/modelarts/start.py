...
# ModelArts平台默认路径
MA_DATA_DIR = '/cache/dataset_efficientnet_b0'

CKPT_OUTPUT_PATH = "/cache/ckpt_efficientnet_b0"
PB_OUTPUT_PATH = "/cache/pb_efficientnet_b0"
CUR_PATH = os.path.dirname(os.path.realpath(__file__))

# 模型输出目录
flags.DEFINE_string('train_url', default=None,
                    help='the path model saved')
# 数据集目录
flags.DEFINE_string('data_url', default=None,
...
flags.DEFINE_string(
    'data_dir', default=MA_DATA_DIR,
    help=('The directory where the ImageNet input data is stored. Please see'
          ' the README.md for the expected data format.'))

flags.DEFINE_string(
    'model_dir', default=CKPT_OUTPUT_PATH,
...
flags.DEFINE_bool('freeze_pb', default=True, help='freeze ckpt to pb model')
...
def find_latest_ckpt(ckpt_dir=CKPT_OUTPUT_PATH):
    """find latest ckpt model file name under ckpt_dir"""
...
def freeze_ckpt_to_pb():
    ...
    num_classes = "--num_classes=" + str(FLAGS.num_label_classes)
    ...
    if not res_file_list or 'efficientnet-b0_tf.pb' not in res_file_list:
        print("freeze ckpt to pb failed: {}".format(res_file_list))
        return

    # 拷贝pb模型到模型输出目录
    mox.file.copy_parallel(PB_OUTPUT_PATH, FLAGS.train_url)
...
def main(argv):
    """main: argv must exist, cannot be removed"""
    set_env()
    mox.file.copy_parallel(FLAGS.data_url, FLAGS.data_dir)
    # modelarts平台上只能指定restore_path的ckpt模型名字，所以需要拼接完整路径
    if FLAGS.restore_path:
        FLAGS.restore_path = os.path.join(FLAGS.data_dir, 'ckpt_pretrained', FLAGS.restore_path)
        match_rule = FLAGS.restore_path + "*"
