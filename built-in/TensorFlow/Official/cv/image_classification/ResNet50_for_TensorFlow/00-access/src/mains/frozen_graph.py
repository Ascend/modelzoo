...
def main():
    ...
    with res50_helper.custom_getter_with_fp16_and_weight_decay(dtype= tf.float32, weight_decay=0.0001):
        builder = resnet.LayerBuilder(tf.nn.relu, 'channels_last', False, use_batch_norm=True,
                               conv_initializer=tf.variance_scaling_initializer(), bn_init_mode='adv_bn_init', bn_gamma_initial_value=1.0)
        logits = resnet.inference_resnet_v1_impl(builder, inputs, [3, 4, 6, 3], 'original', 'v1.5')
    ...
    with tf.Session() as sess:
        ...
        freeze_graph.freeze_graph(
                        ...
                        output_graph='./resnet50_910.pb',   # output pb name
                        clear_devices=False,
                        initializer_nodes='')
    print("done")
