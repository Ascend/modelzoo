...
int main(int argc, char* argv[])
{
    ...
    initParam.modelPath = "../models/inceptionv4_tf_aipp.om";
    auto inceptionv4 = std::make_shared<Inceptionv4ClassifyOpencv>();
    APP_ERROR ret = inceptionv4->Init(initParam);
    if (ret != APP_ERR_OK) {
        LogError << "Inceptionv4ClassifyOpencv init failed, ret=" << ret << ".";
        return ret;
    }
    ...
    for (auto & entry : fs::directory_iterator(imgDir)) {
        ...
        if (ret != APP_ERR_OK) {
            LogError << "Inceptionv4ClassifyOpencv process failed, ret=" << ret << ".";
            inceptionv4->DeInit();
            return ret;
        }
    }
