...
{
    if (argc <= 1) {
        LogWarn << "Please input image path, such as './densenet121 /path/to/jpeg_image_dir'.";
        return APP_ERR_OK;
    }
    ...
    initParam.modelPath = "../models/densenet121_tf_aipp.om";
    auto densenet = std::make_shared<DenseNet121Classify>();
    ret = densenet->Init(initParam);
    if (ret != APP_ERR_OK) {
    ...
    double fps = 1000.0 * imgFiles.size() / densenet->GetInferCostTimeMs();
