...
#基础参数，需要模型审视修改
#网络名称，同目录名称
Network="SWA_ID0252_for_TensorFlow"
#训练epoch
train_epochs=100
#训练step
train_steps=1000
...
do
    ...
    nohup python3 train-tf.keras-npu.py > ${cur_path}/output/${ASCEND_DEVICE_ID}/train_${ASCEND_DEVICE_ID}.log 2>&1 &
if [ $? -ne 0 ];then
  exit 1
fi
done
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
tmp_TrainingTime=`grep "us/sample" $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | tail -1 | grep -Eo "[0-9]*us" | awk -F "u" '{print $1}'`
...
#cat $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log |tr '\b\r' '\n' |grep "1000/1000"| grep -Eo "loss: [0-9]*\.[0-9]*" |awk -F " " '{print $2}' > $cur_path/output/$ASCEND_DEVICE_ID/${CaseName}_loss.txt
grep "loss:"  $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log | grep -Eo "loss: [0-9]*\.[0-9]*" | grep -Eo "[0-9]*\.[0-9]*" >  $cur_path/output/$ASCEND_DEVICE_ID/${CaseName}_loss.txt
#最后一个迭代loss值，不需要修改
