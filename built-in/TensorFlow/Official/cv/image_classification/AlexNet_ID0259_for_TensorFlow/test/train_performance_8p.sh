...
do
    ...
    nohup ${bind_core} python3 main.py --step=600 --mul_rank_size=${RANK_SIZE} \
...
#打印，不需要修改
#TrainingTime=NA
FPS=`grep epoch $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|grep FPS|awk '{print $NF}'|awk '{sum+=$1} END {print '$RANK_SIZE'*sum/NR}'`
