...
class LARSOptimizerNPU(optimizer_v2modified.OptimizerV2Modified):
  ...
  def __init__(
    ...
    Args:
      ...
      name: Optional name prefix for variables and ops created by LARSOptimizerNPU.
    ...
    super(LARSOptimizerNPU, self).__init__(name=name, **kwargs)
  ...
  def compute_lr_npu(self, grad, var, coefficients):
    print('cgf----LARSOptimizerNPU----compute_lr_npu----in----')
    if self._skip_list is None or not any(v in var.name
                                          for v in self._skip_list):
      grad = gen_npu_ops.lars_v2(input_weight=var, input_grad=grad, weight_decay=self.weight_decay,learning_rate=coefficients["learning_rate"], hyperpara=self.eeta, epsilon=self.epsilon)
    return grad
  ...
  def _resource_apply_dense(self, grad, var, apply_state=None):
    ...
    grad = self.compute_lr_npu(grad, var, coefficients)
    mom = self.get_slot(var, "momentum")
  ...
  def get_config(self):
    config = super(LARSOptimizerNPU, self).get_config()
    config.update({
