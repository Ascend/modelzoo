...
function exec_train() {
    ...
    cd $currentDir/test/
    bash train_full_8p_256bs_SGD.sh
    FPS=`grep TimeHistory  $currentDir/output/1/train_1.log|awk 'END {print $6}'`
    train_accuracy=`grep eval_accuracy $currentDir/output/1/train_1.log|grep -v mlp_log|awk 'END {print $5}'| sed 's/,//g' |cut -c 1-5`
