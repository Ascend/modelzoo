...
def parse_record(raw_record, is_training, height, width, config):
  image_buffer, label, bbox = _parse_example_proto(raw_record)
  # for 1980 only
  image = preprocessing.parse_and_preprocess_image_record(
      config, image_buffer, height=height, width=width,
      brightness=config['brightness'], contrast=config['contrast'], saturation=config['saturation'], hue=config['hue'],
      distort=is_training, nsummary=10, increased_aug=False, random_search_aug=False)
  return image, label


def read_record(raw_record):
  image_buffer, label, _ = _parse_example_proto(raw_record)
  return image_buffer, label


def img_transform(image_buffer, label, is_training, height, width, config):
  image = preprocessing.parse_and_preprocess_image_record(
      config, image_buffer, height=height, width=width,
      brightness=config['brightness'], contrast=config['contrast'], saturation=config['saturation'], hue=config['hue'],
      distort=is_training, nsummary=10, increased_aug=False, random_search_aug=False)
...
def parse_function(filename, label, height, width, config):
...
def make_dataset(config, filenames, take_count, batch_size, height, width,
    ...
    if synthetic and training:
        ...
        if config['data_type'] == 'RAW DATA':
            ...
            # parallel read data vs serial read data
        ...
        if config['data_type'] == 'RAW DATA':
            ds = ds.map(lambda image, label: parse_function(
                image, label, height, width, config), num_parallel_calls=14)
        else:
            ds = ds.map(lambda image_row, xxxx: parse_record(image_row, training, height, width, config), num_parallel_calls=216)
        ds = ds.batch(batch_size, drop_remainder=True)
        ds = ds.prefetch(buffer_size=tf.contrib.data.AUTOTUNE)
        # threadpool to manage the call with different threads
