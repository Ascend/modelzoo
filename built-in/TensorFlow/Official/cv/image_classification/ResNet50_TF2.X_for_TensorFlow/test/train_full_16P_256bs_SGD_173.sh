...
if [[ $1 == --help || $1 == -h ]];then
    echo"usage:./train_full_16P_256bs_SGD_173.sh <args>"
...
do
    ...
    elif [ -d "${BASE}/output/ckpt_bs256_SGD_16P_${i}" ];then
        rm -rf ${BASE}/output/ckpt_bs256_SGD_16P_${i}
    fi
    ...
    nohup taskset -c $a-$c python3 resnet_ctl_imagenet_main.py --data_dir=${data_path} --train_steps=${train_steps} --num_accumulation_steps=1 --train_epochs=${train_epochs} -model_dir=${BASE}/output/ckpt_bs256_SGD_16P_$i --distribution_strategy=off --use_tf_while_loop=true --use_tf_function=true --enable_checkpoint_and_export --steps_per_loop=${loop_size} --base_learning_rate=${learning_rate} --momentum=0.901 --epochs_between_evals=1 --eval_offset_epochs=2 --optimizer=SGD --label_smoothing=0.1 --single_l2_loss_op --warmup_epochs=5 --weight_decay=0.000025 --lr_schedule=polynomial --drop_eval_remainder=True --precision_mode=${precision_mode} --over_dump=${over_dump} --data_dump_flag=${data_dump_flag} --data_dump_step=${data_dump_step} --batch_size=${batch_size} --profiling=${profiling} --data_save_path=${data_save_path} > ${BASE}/output/${i}/train_${i}.log 2>&1 &
done
wait
