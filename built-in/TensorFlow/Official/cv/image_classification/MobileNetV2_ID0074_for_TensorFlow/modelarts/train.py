...
def parse_args():
    ...
    tf.app.flags.DEFINE_integer('max_train_steps', 200, 'max steps to train')
...
def set_config():
    ...
    FLAGS.log_dir = '/cache/ckpt_first'


def model_trans():
    match_rule = "model.ckpt-[!0]*.meta*"
    pb_name = "mobileNetv2.pb"

    # set model param
    placed_match_path = os.path.join(FLAGS.log_dir, match_rule)
    ...
    with tf.Session() as sess:
        ...
        freeze_graph.freeze_graph(
            ...
            output_graph=pb_name,   # graph outputs name
            clear_devices=False,
            initializer_nodes='')

    shutil.copy(pb_name, FLAGS.log_dir)
...
def main(_):
    ...
    model_trans()
    mox.file.copy_parallel(FLAGS.log_dir, FLAGS.train_url)
