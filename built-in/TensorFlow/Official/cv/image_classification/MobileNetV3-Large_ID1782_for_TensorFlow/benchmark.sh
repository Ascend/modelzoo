...
function exec_train() {
    ...
    FPS=`grep 'logger.py:56' $currentDir/test/output/1/train_1.log|awk 'END {print $7}' |awk -F ":" '{print $2}'| awk -F "." '{print $1}'`
    train_accuracy=`grep acc: $currentDir/test/output/1/eval_1.log|awk -F '[' '{print $2}' | awk -F ']' '{print $1}' `
