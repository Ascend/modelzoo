...
class LogSessionRunHook(tf.train.SessionRunHook):
    def __init__(self):
        ...
        self.global_batch_size = FLAGS.train_batch_size * FLAGS.rank
        self.iterations_per_loop = FLAGS.iterations_per_loop
        self.iter_times = []
        self.num_records = FLAGS.num_training_samples
        self.display_every = FLAGS.display_every
        self.logger = get_logger(FLAGS.log_name, FLAGS.train_logdir)
    ...
    def before_run(self, run_context):
        ...
        return tf.train.SessionRunArgs(
            fetches=[tf.train.get_global_step(), 'loss:0', 'reg_loss:0', 'total_loss:0', 'learning_rate:0'])
#
    def after_run(self, run_context, run_values):
        ...
        self.count += 1
        global_step, loss, reg_loss, total_loss, lr = run_values.results
        if global_step == 1 or global_step % self.display_every == 0:
            ...
            self.logger.info('step:%6i  epoch:%5.1f  FPS:%7.1f  loss:%3.6f  reg_loss:%2.7f  total_loss:%3.6f  lr:%7.5f' %
                             (global_step, epoch, img_per_sec,loss, reg_loss, total_loss, lr)    )
            self.elapsed_secs = 0.
            self.count = 0
