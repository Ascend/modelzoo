...
**描述（Description）：基于TensorFlow框架的ShapeNet网络， 学习在2D图像空间中分割3D点云**
...
- 开始训练。
    ...
    2. 单卡训练
        ...
        2.1 设置单卡训练参数（脚本位于./ShapeNet_ID1138_for_TensorFlow/test/train_performance_1p.sh），示例如下。请确保下面例子中的“--data_dir”修改为用户数据集的路径。
        
        ```
        `data_path="ShapeNet_prepro.hdf5"`
        ```
        
        
        
        2.2 单卡训练指令（脚本位于./ShapeNet_ID1138_for_TensorFlow/test/train_performance_1p.sh） 
