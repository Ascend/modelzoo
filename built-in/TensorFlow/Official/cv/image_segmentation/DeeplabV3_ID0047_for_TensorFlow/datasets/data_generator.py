...
_COCO2017_SEG_INFORMATION = DatasetDescriptor(
    splits_to_sizes={
        'coco2017_train': 117266,
...
_DATASETS_INFORMATION = {
    ...
    'coco2017':_COCO2017_SEG_INFORMATION,
}
...
class Dataset(object):
  ...
  def get_one_shot_iterator(self):
    """Gets an iterator that iterates across the dataset once.
       ...
       do not support for the cases when iterations_per_loop > 1
       for iterations_per_loop>1 ==> use get_iterator()
    Returns:
  ...
  def _parse_function_v2(self, example_proto):
    """Function to parse the example proto.
  ...
  def _preprocess_image_v2(self, image, label):
    """Preprocesses the image and label.
  ...
  def get_iterator(self):
    ...
    files = self._get_all_files()
    ds = tf.data.TFRecordDataset.list_files(files, shuffle=False)
    ...
    # shuffle and repeat forever for training
    if self.should_repeat:
        ds = ds.apply(tf.data.experimental.shuffle_and_repeat(1000))
    else:
        ds = ds.repeat(1)
