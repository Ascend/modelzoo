...
do
    ...
    --fine_tune_batch_norm=true \
    --tf_initial_checkpoint=${cur_path}/output/s8/r1/0/resnet_101/model.ckpt-15000 \
    --log_steps=100 \
...
python3.7 $cur_path/../train_npu.py \
    ...
    --aspp_with_separable_conv=False \
    --checkpoint_dir=${cur_path}/output/s8/r2/0/resnet_101 \
    --dataset_dir=${data_path}/tfrecord \
    --max_number_of_evaluations=1 > ${cur_path}/output/0/train_0.log 2>&1
cd -
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
FPS=`grep 'FPS:' ${cur_path}/output/s8/r2/0/train_0.log| awk -F "=" '{print $1}' | awk 'END {print $6}'`
#打印，不需要修改
echo "Final Performance images/sec : $FPS"

#输出训练精度,需要模型审视修改
train_accuracy=`grep 'class_20' ${cur_path}/output/0/train_0.log| awk -F " " '{print $3}'`
#打印，不需要修改
...
grep 'loss:' ${cur_path}/output/s8/r2/0/train_0.log|awk '{print $7}' | awk -F "loss:" '{print $2}' >> $cur_path/output/0/train_${CaseName}_loss.txt
