...
do
    ...
    nohup python3.7 $cur_path/../train_npu.py \
...
echo "------------------ Final result ------------------"
#输出性能FPS，需要模型审视修改
FPS=`grep 'FPS:' $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log| awk -F "=" '{print $1}' | awk 'END {print $6}'`
#打印，不需要修改
...
grep 'loss:' $cur_path/output/$ASCEND_DEVICE_ID/train_$ASCEND_DEVICE_ID.log|awk '{print $7}' | awk -F "loss:" '{print $2}' >> $cur_path/output/$ASCEND_DEVICE_ID/train_${CaseName}_loss.txt
