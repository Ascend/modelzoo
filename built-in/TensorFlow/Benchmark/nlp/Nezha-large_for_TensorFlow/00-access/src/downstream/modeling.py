...
class BertModel(object):
  ...
  def __init__(self,
    ...
    with tf.variable_scope(scope, default_name="bert", custom_getter=get_custom_getter(compute_type)):
      ...
      #if tf.flags.FLAGS.use_fp16_cls_mlm:
      if False:
...
def _generate_relative_positions_embeddings(length, depth,
  ...
  with tf.variable_scope(name):
    ...
    if use_one_hot_embeddings:
        if cache:
            raise ValueError("_generate_relative_positions_embeddings does not support cache=True when use_one_hot_embeddings=True")
        flat_relative_positions_matrix = tf.reshape(relative_positions_matrix, [-1])
        one_hot_relative_positions_matrix = tf.one_hot(flat_relative_positions_matrix, depth=vocab_size)
        ...
        my_shape.append(depth)
        embeddings = tf.reshape(embeddings, my_shape)
        print("== one_hot", name, embeddings)
    else:
        embeddings = tf.gather(embeddings_table, relative_positions_matrix)
        print("== gather", name, embeddings)
    return embeddings
  '''
