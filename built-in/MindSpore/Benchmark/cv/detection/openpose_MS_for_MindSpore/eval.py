...
def parse_args():
    ...
    parser.add_argument('--model_path', type=str, default='./outputs/41ap/0-29_150000.ckpt', help='path of testing model')
    parser.add_argument('--imgpath_val', type=str, default='./dataset/val2017', help='path of testing imgs')
    parser.add_argument('--ann', type=str, default='', help='path of annotations')
...
def compute_connections(pafs, all_peaks, img_len, params):
    all_connections = []
...
def eval():
    ...
    sum = dataset.get_dataset_size()
    de_dataset = dataset.create_tuple_iterator()
    
    print("sum=",sum)
    img_paths = getImgsPath(args.imgpath_val)
    print("load dataset right")
    num = len(img_paths)
    kpt_json = []

    #for i, img_path in tqdm(enumerate(img_paths),total = num):
    for i, (img, img_id) in tqdm(enumerate(de_dataset),total = sum):
