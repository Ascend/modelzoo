...
# from mindvision.common.datasets.sampler import DistributedSampler
...
class txtdataset():
    ...
    def generate_labels(self, img, poses, ignore_mask):
        ...
        #return resized_img, pafs, heatmaps, ignore_mask.astype('f')
...
def mindrecord( per_batch_size, data_dir ,max_epoch = 50, drop_remainder=True):
    de_dataset = de.MindDataset(dataset_file=data_dir, num_parallel_workers=48, columns_list=["image", "pafs", "heatmaps", "ignore_mask"],shuffle = True)

    de_dataset = de_dataset.batch(per_batch_size, num_parallel_workers=48, drop_remainder=True)
...
def openpose(jsonpath, imgpath, maskpath, per_batch_size, max_epoch, rank, group_size ,mode = 'train'):
    ...
    sampler = DistributedSampler(dataset, rank, group_size)
    de_dataset = de.GeneratorDataset(dataset, ["image", "pafs", "heatmaps", "ignore_mask"],num_parallel_workers=num_parallel,sampler=sampler,shuffle = True)
...
def evaldata(jsonpath, imgpath, maskpath,mode = 'eval'):
   
    eval = ReadJson(jsonpath)
    num_parallel = 5
    dataset = txtdataset(eval, imgpath, maskpath, params['insize'], mode=mode)
    de_dataset = de.GeneratorDataset(dataset, ["image", "annotations", "img_id"],num_parallel_workers=num_parallel,shuffle = True)
    de_dataset = de_dataset.project(columns = ["image", "annotations", "img_id"])
    de_dataset = de_dataset.batch(batch_size = per_batch_size,drop_remainder=True,num_parallel_workers=num_parallel)
    steap_pre_epoch = de_dataset.get_dataset_size()
    de_dataset = de_dataset.repeat(max_epoch)
    return de_dataset,steap_pre_epoch
