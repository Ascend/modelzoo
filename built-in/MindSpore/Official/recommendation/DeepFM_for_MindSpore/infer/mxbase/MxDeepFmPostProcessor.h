...
namespace {
const float DEFAULT_SCORE_THRESH_DEEPFM = 0.2;
const float DEFAULT_IOU_THRESH_DEEPFM = 0.5;

}  // namespace

class MSDeepfmPostProcessor : public MxBase::ObjectPostProcessorBase {
   public:
    virtual APP_ERROR Init(const std::string &configPath,
   ...
   private:
    ...
    APP_ERROR check_ms_model_compatibility() {
        return APP_ERR_OK;
    }
   ...
   private:
    ...
    float m_fScoreThresh_ = DEFAULT_SCORE_THRESH_DEEPFM;
};
