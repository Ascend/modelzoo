...
def generate_data(output_path, label_dim, number_examples,
                  dense_dim, slot_dim, vocabulary_size, random_slot_values):
    """
    ...
    with open(output_path, 'w') as fp:
        for i in range(number_examples):
            ...
            if (i + 1) % 10000 == 0:
                ...
                start = time.time()
                print(
                    "Processed {} examples with speed {:.2f} examples/s".format(i + 1, speed))
                fp.write('\n'.join(buffer_data) + '\n')
                buffer_data = []
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=str,
        default="./train.txt",
        help='The output path of the generated file')
    parser.add_argument(
        "--label_dim",
        type=int,
        default=2,
        help='The label category')
    parser.add_argument(
        "--number_examples",
        type=int,
        default=4000000,
        help='The row numbers of the generated file')
    parser.add_argument(
        "--dense_dim",
        type=int,
        default=13,
        help='The number of the continue feature.')
    parser.add_argument("--slot_dim", type=int, default=26,
                        help="The number of the category features")
