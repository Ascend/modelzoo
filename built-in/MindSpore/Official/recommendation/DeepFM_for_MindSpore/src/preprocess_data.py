...
class StatsDict(object):
    ...
    def __init__(self, field_size, dense_dim, slot_dim, skip_id_convert):
        ...
        self.cat_count_dict = {
            col: collections.defaultdict(int) for col in self.cat_cols}

        self.oov_prefix = "OOV"

        self.cat2id_dict = {}
        self.cat2id_dict.update(
            {col: i for i, col in enumerate(self.val_cols)})
        self.cat2id_dict.update(
        ...
        for i, cat_str in enumerate(cats):
            ...
            if key in self.cat2id_dict:
                if self.skip_id_convert is True:
                    ...
                    # problem is skip the id convert, regarding the synthetic
                    # data id as the final id.
                    id_list.append(cat_str)
                else:
                    id_list.append(self.cat2id_dict[key])
            else:
                id_list.append(
                    self.cat2id_dict[self.oov_prefix + "cat_{}".format(i + 1)])
...
def mkdir_path(file_path):
    ...
    if not os.path.exists(file_path):
        ...
        print("%s has exist, will delete it and remake" % file_path)
        shutil.rmtree(file_path)
        os.makedirs(file_path)


def statsdata(file_path, dict_output_path,
              recommendation_dataset_stats_dict, dense_dim=13, slot_dim=26):
...
def random_split_trans2mindrecord(input_file_path, output_file_path, recommendation_dataset_stats_dict,
    ...
    writer_train = FileWriter(
        os.path.join(
            output_file_path,
            "train_input_part.mindrecord"),
        21)
    writer_test = FileWriter(
        os.path.join(
            output_file_path,
            "test_input_part.mindrecord"),
        3)
    ...
    print("-------------" * 10)
    print(
        "items_error_size_lineCount.size(): {}.".format(
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--data_path",
        type=str,
        default="./recommendation_dataset/",
        help='The path of the data file')
    parser.add_argument(
        "--dense_dim",
        type=int,
        default=13,
        help='The number of your continues fields')
