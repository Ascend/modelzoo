...
class CDvppImage {
   public:
    CDvppImage(MxBase::DvppWrapper *dvpp) : m_pDvppWrapper(dvpp) {}
    ~CDvppImage() { dispose(); }
    ...
    void dispose() {
        if (*this) {
            m_oImageInfo.destory(&m_oImageInfo.data);
...
class CVImage {
   public:
    ...
    size_t FetchImageBytes() { return m_oImage.total() * m_oImage.elemSize(); }
