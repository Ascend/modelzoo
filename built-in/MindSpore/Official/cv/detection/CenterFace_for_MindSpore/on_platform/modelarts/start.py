...
# network related
parser.add_argument('--pretrained_backbone', default='/tmp/backbone/mobilenet_v2.ckpt', type=str,
                    help='model_path, local pretrained backbone model to load')
parser.add_argument(
    '--resume',
    default='/tmp/resume/',
    type=str,
...
if __name__ == "__main__":
    ...
    if os.path.isfile(args.resume):
        ...
        resumed_ckpt = os.path.join(resume_path, resume_ckpt_name)
        mox.file.copy(args.resume, resumed_ckpt)
        resume_epoch = get_epoch(resume_ckpt_name)
        args.max_epoch = args.max_epoch - resume_epoch
        args.lr_epochs = np.array(args.lr_epochs) - resume_epoch
        ...
        if (i_all + 1) % args.steps_per_epoch == 0:
            ...
            if args.rank == 0:
                args.logger.info(
                    ...
                    .format(epoch - resume_epoch, loss_meter,
                            fps, lr[i + (epoch - resume_epoch - 1) * args.steps_per_epoch])
                )
            t_end = time.time()
            loss_meter.reset()
            mox.file.copy_parallel(args.outputs_dir, args.train_url)
    ...
    args.logger.info('==========start export===============')

    rank_size = int(os.environ.get("RANK_SIZE", 1))
    rank_id = int(os.environ.get('RANK_ID'))

    def model_tran(args, ckpt_path):
        """
        model tran
        """
        context.set_context(
        ...
        net.set_train(False)
        input_data = Tensor(
            np.zeros([1, 3, config.input_h, config.input_w]), mindspore.float32)
        export(
            net,
            input_data,
    ...
    if rank_size > 1:
        if rank_id == 0:
            model_tran(args, args.outputs_dir)
    else:
        model_tran(args, args.outputs_dir)
