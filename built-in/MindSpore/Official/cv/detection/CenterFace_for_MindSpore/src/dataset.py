...
def GetDataLoader(per_batch_size,
    ...
    sampler = DistributedSampler(
        centerface_gen, rank, group_size, shuffle=(
            split == 'train'))  # user defined sampling strategy
    de_dataset = ds.GeneratorDataset(
        centerface_gen, [
            "image", "anns"], sampler=sampler, num_parallel_workers=16)
    ...
    if split == 'train':
        # de_dataset = de_dataset.repeat(1) # if use this, need an additional
        # "for" cycle epoch times
