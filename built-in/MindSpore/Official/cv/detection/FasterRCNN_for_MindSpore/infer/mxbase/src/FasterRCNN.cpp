...
APP_ERROR FasterRCNN::Init(const InitParam &initParam) {
    ...
    configData.SetJsonValue("SCORE_THRESH", std::to_string(initParam.scoreThresh));
    ...
    post_ = std::make_shared<MxBase::FasterRcnnMindsporePost>();
...
APP_ERROR FasterRCNN::ReadImage(const std::string &imgPath, MxBase::DvppDataInfo &output, ImageShape &imgShape) {
...
APP_ERROR FasterRCNN::Resize(const MxBase::DvppDataInfo &input, MxBase::TensorBase &outputTensor) {
    ...
    output.destory = (void (*)(void *))memoryData.free;
...
APP_ERROR FasterRCNN::Inference(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR FasterRCNN::PostProcess(const std::vector<MxBase::TensorBase> &inputs,
...
APP_ERROR FasterRCNN::GetImageMeta(const ImageShape &imageShape, TensorBase &imgMetas) const {
    float widthScale = 0;
    float heightScale = 0;
    ...
    size_t metaSize = sizeof(float) * 4;
    auto *im_info = reinterpret_cast<float *>(malloc(metaSize));
    im_info[0] = static_cast<float>(imageShape.height);
    im_info[1] = static_cast<float>(imageShape.width);
    ...
    MxBase::MemoryData memoryData(imInfo_dst, metaSize, MemoryData::MemoryType::MEMORY_DEVICE, deviceId_);
    memoryData.free = aclrtFree;

    const std::vector<uint32_t> shape = {1, 4};
    imgMetas = TensorBase(memoryData, false, shape, TENSOR_DTYPE_FLOAT32);
    LogInfo << "ImgMetas: " << imgMetas.GetByteSize();
    return APP_ERR_OK;
}

void SaveInferResult(const std::vector<ObjectInfo> &objInfos, const std::string &resultPath) {
    if (objInfos.empty()) {
...
APP_ERROR FasterRCNN::Process(const std::string &imgPath, const std::string &resultPath) {
