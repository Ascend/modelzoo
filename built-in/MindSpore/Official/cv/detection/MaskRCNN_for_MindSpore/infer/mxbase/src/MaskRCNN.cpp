...
APP_ERROR MaskRCNN::GetImageMeta(const ImageShape &imageShape, TensorBase &imgMetas) const {
    float widthScale = 0;
    float heightScale = 0;
    ...
    imgMetas = TensorBase(memoryData, false, shape, TENSOR_DTYPE_FLOAT16);
...
void SaveInferResult(const std::vector<ObjectInfo> &objInfos, const std::string &resultPath) {
    ...
    for (auto &obj : objInfos) {
        ...
        std::string maskStr;
        std::cout << "Mask as fallows:" << std::endl;
