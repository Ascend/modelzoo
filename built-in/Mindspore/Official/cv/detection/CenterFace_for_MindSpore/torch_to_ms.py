...
# ============================================================================
"""
centerface model transform: torch <=> mindspore
"""
import torch
import os
import numpy as np
...
def ckpt_to_pt(pt, ckpt, out_path):
    ...
    for key in state_dict_torch:
        ...
        if state_dict_ms[name_relate[key]].data.shape != state_dict_torch[key].numpy().shape:
            print('before ms=', state_dict_ms[name_relate[key]].data.shape, 'pytorch=', state_dict_torch[key].numpy().shape, 'name=',key)
            parameter = parameter.transpose(1, 0, 2, 3)
            print('after ms=', state_dict_ms[name_relate[key]].data.shape, 'pytorch=', state_dict_torch[key].numpy().shape, 'name=',key)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--pt_fn', type=str, default='/model_path/centerface.pth',
    ...
    parser.add_argument('--pt2ckpt', type=int, default=1,
