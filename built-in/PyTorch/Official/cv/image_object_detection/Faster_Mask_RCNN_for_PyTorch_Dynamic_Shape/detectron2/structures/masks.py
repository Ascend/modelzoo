...
class BitMasks:
    ...
    def crop_and_resize(self, boxes: torch.Tensor, mask_size: int) -> torch.Tensor:
        ...
        boxes = torch.stack([boxes[:, 1], boxes[:, 0], boxes[:, 3], boxes[:, 2]], 1)
        output = (
            torch.npu_crop_and_resize(bit_masks[:, None, :, :], boxes.float() / self.image_hwhw, batch_inds,
                                      torch.tensor([mask_size, mask_size], device=device, dtype=torch.int32))
            .squeeze(1)
        )
        output = output >= 0.5
        return output
