...
class TransformerModel(nn.Module):
    ...
    def fwd(self, x, lengths, causal, src_enc=None, src_len=None, positions=None, langs=None, cache=None):
        ...
        tensor = tensor.half().npu_format_cast(2)
        ...
        mask2 = mask.view(-1).unsqueeze(-1).expand(-1, self.dim)

        # transformer layers
        for i in range(self.n_layers):

            # self attention
            attn = self.attentions[i](tensor, bs, qlen, attn_mask, cache=cache)
            ...
            if self.is_decoder and src_enc is not None:
                attn = self.encoder_attn[i](tensor, bs, qlen, src_mask, kv=src_enc, cache=cache)
                attn = F.dropout(attn, p=self.dropout, training=self.training)
                tensor = tensor + attn
    ...
    def predict(self, tensor, pred_mask, y, get_scores):
        ...
        masked_tensor = (tensor * (pred_mask.unsqueeze(-1).expand_as(tensor))).contiguous().view(-1, self.dim)
