...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, query: Tensor, key: Tensor, value: Tensor,
        ...
        q = q.contiguous().view(tgt_len, bsz * self.num_heads, self.head_dim).transpose(0, 1).contiguous()
        k = k.contiguous().view(src_len, bsz * self.num_heads, self.head_dim).transpose(0, 1).contiguous()
        v = v.contiguous().view(src_len, bsz * self.num_heads, self.head_dim).transpose(0, 1).contiguous()
