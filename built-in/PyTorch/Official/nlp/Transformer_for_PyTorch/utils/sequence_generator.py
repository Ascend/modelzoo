...
from models import IncrementalDecoder
...
class SequenceGenerator(object):
    def __init__(
        self, args,models, vocab_meta, maxlen, beam_size=1, minlen=1, stop_early=True,
    ...
    def generate_batched_itr(
        ...
        npu=False, timer=None, prefix_size=0,
    ):
        ...
        for sample in data_itr:
            s = utils.move_to_npu(self.args,sample) if npu else sample
    ...
    def _generate(self, src_tokens, src_lengths, beam_size=None, maxlen=None, prefix_tokens=None):
        ...
        for model in self.models:
            ...
            s_r = src_tokens.repeat(1, beam_size).view(-1, srclen)
            s_l = src_lengths.expand(beam_size, src_lengths.numel()).t().contiguous().view(-1)
        ...
        # initialize buffers
        scores = src_tokens.data.new_empty(bsz * beam_size, maxlen + 1).float().fill_(0)
        scores_buf = scores.clone()
        tokens = src_tokens.data.new_empty(bsz * beam_size, maxlen + 2).fill_(self.pad)
        tokens_buf = tokens.clone()
        tokens[:, 0].fill_(self.eos)
        ...
        worst_finalized = [{'idx': None, 'score': torch.finfo(torch.float32).min} for i in range(bsz)]
        ...
        def finalize_hypos(step, bbsz_idx, eos_scores, unfinalized_scores=None):
            ...
            tokens_clone = tokens_clone.float()
            tokens_clone[:, step].fill_(self.eos)
            tokens_clone = tokens_clone.int()
            ...
            for i, (idx, score) in enumerate(zip(bbsz_idx.tolist(), eos_scores.tolist())):
                ...
                def get_hypo():
                    ...
                    if attn_clone is not None:
                        ...
                        # print('b', b.size())
                        hypo_attn = b[a]
                        # hypo_attn = hypo_attn.float()
                        _, alignment = hypo_attn.max(dim=0)
                        # hypo_attn = hypo_attn.half()
                    else:
                        print('+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++27')
                        hypo_attn = None
                        alignment = None
        ...
        for step in range(maxlen + 1):  # one extra step for EOS marker
            ...
            if reorder_state is not None:
                if batch_idxs is not None:
                    ...
                    print("corr", corr.shape, corr.type())
            ...
            probs[:, self.pad].fill_(torch.finfo(torch.float32).min)
            probs[:, self.unk] -= self.unk_penalty  # apply unk penalty
            # Record attention scores
            if avg_attn_scores is not None:
                if attn is None:
                    attn = scores.new_empty(bsz * beam_size, src_tokens.size(1), maxlen + 2)
                    attn_buf = attn.clone()
            ...
            if step < maxlen:
                if prefix_tokens is not None and step < prefix_tokens.size(1):
                    ...
                    kk = min(cand_size, probs.view(bsz, -1).size(1) - 1)
                    pv = probs.view(bsz, -1)
                    pv = pv.half()
                    ...
                    if pv.size()[1] > 65500:
                        for i in range(1, beam_size + 1):
                            print("pv", pv[:, 33712 * (i-1):33712 * i].shape,pv[:, 33712 * (i-1):33712 * i].type())
                            cand_scores_, cand_indices_ = torch.topk(pv[:, 33712 * (i-1):33712 * i], k=kk,)
                            cand_indices_ += 33712 * (i - 1)
                            if i == 1:
                                cand_scores = cand_scores_
                                cand_indices = cand_indices_
                            else:
                                cand_scores = torch.cat((cand_scores, cand_scores_), 1)
                                cand_indices = torch.cat((cand_indices, cand_indices_), 1)
                        cand_scores, ci = torch.topk(cand_scores, k=kk,)

                        ci = ci.long()
                        cand_indices = torch.gather(cand_indices, 1, ci)
                    else:
                        cand_scores,cand_indices = torch.topk(pv,k=kk)

                    cand_scores = cand_scores.float()
            ...
            if len(finalized_sents) > 0:
                ...
                temp = cand_indices.new_tensor(finalized_sents)
                batch_mask[temp.long()] = 0
            ...
            ets = eos_mask.type_as(cand_offsets)
            ets = ets.half()
            cao = cand_offsets[:eos_mask.size(1)]
            cao = cao.half()
            active_mask = torch.add(ets * cand_size, cao,)
            ...
            active_hypos = active_hypos.long()
            ...
            print(scores.type(), active_scores.type())
            scores[:, step] = active_scores.view(scores[:, step].size())
            ...
            tokens_buf[:,step + 1] = tga.view(tokens_buf[:,step + 1].size())
            ...
            print(scores_buf.type(), tga1.type())
            scores_buf[:,step] = tga1.view(scores_buf[:,step].size())
